...
{
    public partial class ImportOverStockOrders : BasePage
    {
        #region Declaration
        int totalrecords = 0;
        ...
        private DataTable LoadCSV(string FileName)
        {
            ...
            FileInfo info = new FileInfo(Server.MapPath(AppLogic.AppConfigs("ProductImportPath") + "ProductCSV/ImportOverStockCSV/") + FileName);
        ...
        private void DeleteDocument(string StrFileName)
        {
            try
            {
                string docPath = Server.MapPath(AppLogic.AppConfigs("ProductImportPath") + "ProductCSV/ImportOverStockCSV/") + StrFileName;
        ...
        private void FillMapping(string FileName)
        {
            ...
            using (CsvReader csv = new CsvReader(new StringReader(File), true))
            {
                ...
                if (FieldCount > 0)
                {
                    ...
                    foreach (string FieldName in FieldNames)
                    {
                        string tempFieldName = FieldName.ToUpper();
                        if (tempFieldName == "CREATED" || tempFieldName == "UNIQUE_ID" || tempFieldName == "FIRST_NAME" || tempFieldName == "LAST_NAME" || tempFieldName == "ADDR_LINE_ONE" || tempFieldName == "ADDR_LINE_TWO" || tempFieldName == "CITY" || tempFieldName == "STATE" || tempFieldName == "ZIP" || tempFieldName == "COUNTRY_CODE" || tempFieldName == "EMAIL" || tempFieldName == "PRODUCT_ID" || tempFieldName == "SUPPLIER_SKU" || tempFieldName == "ITEM_NAME")
                    ...
                    {
                        lblMessage.Text = "Please Specify UNIQUE_ID,CREATED,FIRST_NAME,LAST_NAME,ADDR_LINE_ONE,ADDR_LINE_TWO,CITY,STATE,ZIP,COUNTRY_CODE,EMAIL,PRODUCT_ID,SUPPLIER_SKU,PRODUCT_OPTION_ID,ITEM_NAME in file.";
                        lblMessage.Style.Add("color", "#FF0000");
                        lblMessage.Style.Add("font-weight", "normal");
                    }
        ...
        protected void btnImport_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                {
                    lblMessage.Text = "";
                    if (!Directory.Exists(Server.MapPath(AppLogic.AppConfigs("ProductImportPath") + "ProductCSV/ImportOverStockCSV/")))
                        Directory.CreateDirectory(Server.MapPath(AppLogic.AppConfigs("ProductImportPath") + "ProductCSV/ImportOverStockCSV/"));
                    StrFileName = uploadCSV.FileName;
                    DeleteDocument(StrFileName);
                    uploadCSV.SaveAs(Server.MapPath(AppLogic.AppConfigs("ProductImportPath") + "ProductCSV/ImportOverStockCSV/") + StrFileName);
                ...
                if (!string.IsNullOrEmpty(StrFileName))
                {
                    ...
                    dv.Sort = "UNIQUE_ID ASC";

                    DataSet dscsv = new DataSet();
                    dscsv.Tables.Add(dv.ToTable());

                    if (ImportOverStockOrderData(dscsv, 4) && lblMessage.Text == "")
                    {
                        ...
                        {
                            lblImport.Text +=   "<br />Error With SKU :<br />" + allError;
                            lblImport.Visible = true;
                            lblImport.Style.Add("color", "#FF0000");
                            lblImport.Style.Add("font-weight", "normal");
                        }
        ...
        private bool ImportOverStockOrderData(DataSet ds, Int32 StoreID)
        {
            ...
            string strShippinglastName = string.Empty;
                ...
                if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
                {
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        try
                        {
                            ...
                            iAmazonOrderNumber = ds.Tables[0].Rows[i][0].ToString(); //P.O. Number:
                            bool bOrderExists = CheckAmazonOrder(iAmazonOrderNumber, StoreID);
                            try
                            {
                                strShippingMethod = "First Class Mail Parcel";// Convert.ToString(ds.Tables[0].Rows[i]["SHIP_METHOD"].ToString());
                            }
                            catch
                            {
                            ...
                            if (!bOrderExists)
                            {
                                ...
                                //{
                                strCustomerEmail = Convert.ToString(ds.Tables[0].Rows[i]["EMAIL"].ToString());
                                strShippingName = Convert.ToString(ds.Tables[0].Rows[i]["FIRST_NAME"].ToString());
                                strShippinglastName = Convert.ToString(ds.Tables[0].Rows[i]["LAST_NAME"].ToString());
                                strShippingAddress1 = Convert.ToString(ds.Tables[0].Rows[i]["ADDR_LINE_ONE"].ToString());

                                strShippingAddress2 = Convert.ToString(ds.Tables[0].Rows[i]["ADDR_LINE_TWO"].ToString());
                                strShippingAddress3 = "";
                                strShippingCity = Convert.ToString(ds.Tables[0].Rows[i]["CITY"].ToString());
                                strShippingZip = Convert.ToString(ds.Tables[0].Rows[i]["ZIP"].ToString());
                                strShippingCountry = "";
                                //strShippingPhone = Convert.ToString(ds.Tables[0].Rows[i]["Ship To Phone"].ToString());
                                strShippingState = Convert.ToString(CommonComponent.GetScalarCommonData("select Name from tb_State where Abbreviation='" + Convert.ToString(ds.Tables[0].Rows[i]["STATE"].ToString()) + "'"));
                                strShippingCountry = Convert.ToString(CommonComponent.GetScalarCommonData("select isnull(Name,'') from tb_Country where countryid in (select countryid from tb_State where Abbreviation='" + Convert.ToString(ds.Tables[0].Rows[i]["STATE"].ToString()) + "')"));

                                //}


                                CustID = Convert.ToInt32(CommonComponent.GetScalarCommonData("INSERT INTO tb_Customer(StoreID,Email,FirstName,LastName,Active,Deleted,IsRegistered) VALUES (" + StoreID + ",'" + strCustomerEmail + "','" + strShippingName.Replace("'", "''") + "','" + strShippinglastName.Replace("'", "''") + "',0,1,0); SELECT SCOPE_IDENTITY();"));
                                ...
                                DataRow[] drProduct = ds.Tables[0].Select("[" + ds.Tables[0].Columns[0].ColumnName.ToString().Replace(":", "") + "]='" + iAmazonOrderNumber + "'");
                                ...
                                if (drProduct != null && drProduct.Length > 0)
                                {
                                    foreach (DataRow dr in drProduct)
                                    {
                                        ...
                                        Int32 ProductID = Convert.ToInt32(CommonComponent.GetScalarCommonData("select top 1 Productid from tb_product where isnull(Deleted,0)=0 and (sku='" + dr["SUPPLIER_SKU"].ToString().Replace("'", "''").Replace("=", "").Replace("\"", "") + "') and storeid=" + StoreID));
                                        if (ProductID <= 0)
                                        {
                                        ...
                                        dssku = CommonComponent.GetCommonDataSet("EXEC GetoverstockSwatchorder '" + dr["SUPPLIER_SKU"] + "'");
                                        ...
                                        {
                                            if (!string.IsNullOrEmpty(dssku.Tables[0].Rows[0]["SKU"].ToString()))
                                            {

                                                tOrder = true;
                                                Decimal prpp = Convert.ToDecimal(dssku.Tables[0].Rows[0]["price"].ToString());
                                                string Name = Convert.ToString(dssku.Tables[0].Rows[0]["Name"].ToString());
                                              ...
                                              " values(" + orderSID + ",'" + ProductID + "',1," + prpp.ToString() + ",'','','" + Name.ToString().Replace("'", "''") + "','" + dssku.Tables[0].Rows[0]["SKU"].ToString().Replace("'", "''").Replace("=", "").Replace("\"", "") + "','" + dr["PRODUCT_ID"].ToString().Replace("'", "''").Replace("=", "").Replace("\"", "") + "',0)");
                                                OrderSubtotal = OrderSubtotal + Convert.ToDecimal(Convert.ToDecimal(1) * Convert.ToDecimal(prpp));
                                            }
                                            else
                                            {
                                                allError = allError + iAmazonOrderNumber.ToString() + "_" + dr["SUPPLIER_SKU"].ToString() +"<br />";
                                            }
                                        }
                                        //else
                                        //{
                                ...
                                if (tOrder == true)
                                {
                                    ...
                                    Decimal OrderTax = 0;
                                    Int32 OrderNumber = Convert.ToInt32(CommonComponent.GetScalarCommonData(@"insert into tb_Order (GiftCertificateDiscountAmount,orderStatus,StoreID,CustomerID,ShoppingCardID,LastName,FirstName,Email,BillingEmail,BillingLastName,BillingFirstName,BillingCompany,BillingAddress1,BillingAddress2,BillingSuite,BillingCity,BillingState,BillingZip,BillingCountry,BillingPhone,ShippingEmail,ShippingFirstName,ShippingLastName,ShippingCompany,ShippingAddress1,ShippingAddress2,ShippingState,ShippingCity,ShippingZip,ShippingCountry,ShippingPhone,OrderTotal,OrderSubtotal,OrderShippingCosts,OrderTax,Deleted,RefOrderID,PaymentMethod,OrderDate,Isnew,LevelDiscountAmount,CouponDiscountAmount,TransactionStatus,CardType,CardVarificationCode,CardNumber,CardName,CardExpirationMonth,CardExpirationYear,Last4,paymentgateway,CustomDiscount,ShippingMethod,ShippedOn,ShippedVIA,QuantityDiscountAmount)
 
 values(0,'New'," + StoreID + "," + CustID + "," + orderSID + ",'" + strShippinglastName.ToString().Replace("'", "''") + "','" + strShippingName.ToString().Replace("'", "''") + "','" + strCustomerEmail.ToString().Replace("'", "''") + "','" + strCustomerEmail.ToString().Replace("'", "''") + "','" + strShippinglastName.ToString().Replace("'", "''") + "','" + strShippingName.ToString().Replace("'", "''") + "','','" + strShippingAddress1.ToString().Replace("'", "''") + "','" + strShippingAddress2.ToString().Replace("'", "''") + "','','" + strShippingCity.ToString().Replace("'", "''") + "','" + strShippingState + "','" + strShippingZip.ToString().Replace("'", "''") + "','" + strShippingCountry.Replace("'", "''") + "','" + strShippingPhone + "','" + strCustomerEmail.ToString().Replace("'", "''") + "','" + strShippingName.ToString().Replace("'", "''") + "','" + strShippinglastName.ToString().Replace("'", "''") + "','','" + strShippingAddress1.ToString().Replace("'", "''") + "','" + strShippingAddress2.ToString().Replace("'", "''") + "','" + strShippingState.Replace("'", "''") + "','" + strShippingCity.ToString().Replace("'", "''") + "','" + strShippingZip.ToString().Replace("'", "''") + "','" + strShippingCountry.Replace("'", "''") + "','" + strShippingPhone + "','" + OrderTotal + "','" + OrderSubtotal + "','" + Shippingtotal + "','" + OrderTax + "',0,'" + iAmazonOrderNumber.ToString().Replace("'", "''").Trim() + "','CREDITCARD','" + String.Format("{0:MM/dd/yyyy hh:mm:ss ttt}", Convert.ToDateTime(ds.Tables[0].Rows[i]["CREATED"].ToString())) + "',1,0,0,'CAPTURED','','','','','','','','AUTHORIZENET',0,'" + strShippingMethod + "','','','0.0'); SELECT SCOPE_IDENTITY();"));
                    ...
                    int diff = 0;
                    totalrecords = Convert.ToInt32(ds.Tables[0].DefaultView.ToTable(true, "UNIQUE_ID").Rows.Count.ToString()); //Convert.ToInt32(drAll.Length);
