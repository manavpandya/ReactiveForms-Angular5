    ...
    public partial class Invoice : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (Request.QueryString["ONo"] != null)
            {
                ...
                if (Request.Url != null)
                {
                    url = "http://" + Request.Url.Authority + Request.ApplicationPath.TrimEnd('/');
                }
                else
                {
                    url = "http://www.halfpricedrapes.us";
                }
        ...
        private void GetOrderDetails(Int32 OrderNumber)
        {
            ...
            if (objDsorder != null && objDsorder.Tables.Count > 0 && objDsorder.Tables[0].Rows.Count > 0)
            {
                ...
                //{
                //    ltrorderNumber.Text = ltrorderNumber.Text.ToString() + " (Ref. Order No. : " + objDsorder.Tables[0].Rows[0]["RefOrderID"].ToString() + ")";
                //}
                ...
                imgOrderBarcode.Src = url + FPath + "/ONo-" + OrderNumber + ".png";
                ...
                for (int i = 0; i < dsCart.Tables[0].Rows.Count; i++)
                {
                    ...
                    for (int j = 0; j < variantValue.Length; j++)
                    {
                        if (variantName.Length > j)
                        {
                            ...
                            dsoption = objSql.GetDs("SELECT SKU,UPC,Header FROM tb_ProductVariantValue WHERE ProductID=" + dsCart.Tables[0].Rows[i]["RefproductId"] + " AND VariantValue='" + variantValue[j].ToString().ToLower().Replace("(buy 1 get 1 free)", "").Replace("(on sale)", "").Replace("'", "''") + "'");
                            ...
                            if (variantValue[j].ToString().IndexOf("($") > -1)
                            {
                                ...
                                // Response.Write("SELECT SKU,UPC,Header FROM tb_ProductVariantValue WHERE ProductID=" + dsCart.Tables[0].Rows[i]["RefproductId"] + "  and isnull(upc,'')<>'' AND VariantValue='" + strll.ToString().Replace("'", "''").Trim() + "'");
                                dsoption = objSql.GetDs("SELECT SKU,UPC,Header FROM tb_ProductVariantValue WHERE ProductID=" + dsCart.Tables[0].Rows[i]["RefproductId"] + "  and isnull(upc,'')<>'' AND VariantValue='" + strll.ToString().ToLower().Replace("(buy 1 get 1 free)", "").Replace("(on sale)", "").Replace("'", "''") + "'");
                            }
                    ...
                    {
                        DataSet dsoption = new DataSet();
                        dsoption = CommonComponent.GetCommonDataSet("SELECT  UPC FROM tb_Product  WHERE ProductID=" + dsCart.Tables[0].Rows[i]["RefproductId"] + " and isnull(upc,'')<>'' AND SKU='" + dsCart.Tables[0].Rows[i]["CartSKU"].ToString().Replace("'", "''") + "'");
                        ...
                        {
                            dsoption = CommonComponent.GetCommonDataSet("SELECT UPC FROM tb_Product WHERE StoreId=1 and isnull(deleted,0)=0 AND SKU='" + dsCart.Tables[0].Rows[i]["CartSKU"].ToString().Replace("'", "''") + "' and isnull(upc,'')<>''");
                            if (dsoption != null && dsoption.Tables.Count > 0 && dsoption.Tables[0].Rows.Count == 0 && string.IsNullOrEmpty(strupimage))
                            {
                                dsoption = CommonComponent.GetCommonDataSet("SELECT Top 1 SKU,UPC,Header FROM tb_ProductVariantValue WHERE SKU=" + dsCart.Tables[0].Rows[i]["CartSKU"].ToString() + " and isnull(upc,'')<>''");
                            }
                            if (dsoption != null && dsoption.Tables.Count > 0 && dsoption.Tables[0].Rows.Count > 0 && string.IsNullOrEmpty(strupimage))
                            {
                                if (!string.IsNullOrEmpty(dsoption.Tables[0].Rows[0]["UPC"].ToString()))
                    ...
                    else
                    {
                        ...
                        {
                            DataSet dsoption = new DataSet();
                            if (dsCart.Tables[0].Rows[i]["CartSKU"].ToString().Replace("'", "''").ToLower().IndexOf("-cus") > -1)
                            {
                                dsoption = CommonComponent.GetCommonDataSet("SELECT SKU,UPC,Header FROM tb_ProductVariantValue WHERE ProductID=" + dsCart.Tables[0].Rows[i]["RefproductId"] + "  and isnull(upc,'')<>'' AND SKU='" + dsCart.Tables[0].Rows[i]["CartSKU"].ToString() + "'");
                            }
                            if (dsoption != null && dsoption.Tables.Count > 0 && dsoption.Tables[0].Rows.Count > 0)
                            {
        ...
        public void SendMail(Int32 OrderNumber)
        {
            ...
            string url = AppLogic.AppConfigs("LIVE_SERVER") + "/Admin/Orders/" + "Invoicemail.aspx?ONo=" + OrderNumber.ToString();
