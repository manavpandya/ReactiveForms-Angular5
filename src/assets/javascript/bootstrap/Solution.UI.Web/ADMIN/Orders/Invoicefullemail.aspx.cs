...
{
    public partial class Invoicefullemail : BasePage
    {
        /// <summary>
        /// Page Load Event
        /// </summary>
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (Request.QueryString["ONo"] != null)
            {
                ...
                ImgStoreLogo.Src = AppLogic.AppConfigs("LIVE_SERVER").ToString() + "/Images/Store_" + StoreID.ToString() + ".png";
            }
        ...
        private void GetOrderDetails(Int32 OrderNumber)
        {
            ...
            if (objDsorder != null && objDsorder.Tables.Count > 0 && objDsorder.Tables[0].Rows.Count > 0)
            {
                ...
                imgOrderBarcode.Src = AppLogic.AppConfigs("LIVE_SERVER").ToString() + FPath + "/ONo-" + OrderNumber + ".png";
                    ...
                    for (int j = 0; j < variantValue.Length; j++)
                    {
                        if (variantName.Length > j)
                        {
                            ...
                            if (dsoption != null && dsoption.Tables.Count > 0 && dsoption.Tables[0].Rows.Count > 0)
                            {
                                ...
                                //    {
                                //        sku += "<br/><img width=\"160px\" src=\"" + AppLogic.AppConfigs("LIVE_SERVER").ToString() + FPath + "/UPC-" + dsoption.Tables[0].Rows[0]["UPC"].ToString().Trim() + ".png" + "\" />";
                                //    }
                                //    else
                                //    {
                                //        if (!System.IO.File.Exists(Server.MapPath(FPath + "/UPC-" + dsoption.Tables[0].Rows[0]["UPC"].ToString().Trim() + ".png")))
