...
{
    public partial class VendorQuote_POMailFormat : BasePage
    {
        DataSet dsTemplate;
        ...
        /// <returns>Returns the output value as a string format which contains HTML.</returns>s
        private String BindVendorCart(string VendorID)
        {
            if (Session["VendorQuotePOCart"] != null)
            {
                ...
                ViewState["VendorQuotePOCart"] = Session["VendorQuotePOCart"];
                DataTable DtVendor = (DataTable)Session["VendorQuotePOCart"];
                DataView dvVendor = DtVendor.DefaultView;
                dvVendor.RowFilter = "vendorIDs='" + VendorID.ToString() + "'";
                ...
                for (int i = 0; i < dvVendor.Count; i++)
                {
                    ...
                    sb.Append(Convert.ToDecimal(dvVendor.Table.Rows[i]["Price"].ToString()).ToString("c"));
                    sb.Append("</td>");
                    sb.Append("</tr>");
        ...
        private String FillMailTemplateTextForVendor(String strText)
        {
            ...
            if (Request.QueryString["Notes"] != null)
            {
                try
                {
                    strSource = Regex.Replace(strSource, "###NOTES###", Session["VendorPONotes"].ToString() == "" ? "N/A" : Session["VendorPONotes"].ToString(), RegexOptions.IgnoreCase);
                }
                catch { }
            }
            else
        ...
        protected void btnSendmailToVendor_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (ViewState["VendorQuotePOCart"] != null)
            {
                DataTable DtVendor = (DataTable)ViewState["VendorQuotePOCart"];
                ...
                Notes = Session["VendorPONotes"].ToString(); //Server.UrlDecode(Request.QueryString["Notes"].ToString());
                ...
                if (PONumber != 0)
                {
                    ...
                    foreach (DataRow dr in DtVendor.Rows)
                    {
                        strQuery.Append(" insert into tb_PurchaseOrderItems(ProductID,Quantity,Price,PONumber,VendorQuoteRequestID,VendorQuoteReqDetailsID,ProductOption) values(" + dr["ProductID"] + "," + dr["Quantity"].ToString() + "," + Math.Round(Convert.ToDecimal(dr["Price"].ToString()), 2) + "," + PONumber + "," + Convert.ToInt32(dr["RequestQuoteId"]) + "," + Convert.ToInt32(dr["RequestQuoteDetailId"]) + ",'" + Convert.ToString(dr["ProductOption"]) + "') ");
                ...
                if (Session["QuoteOrderCart"] != null)
                {
                    ...
                    DataRow[] dataRows = DtVendor.Select("vendorIDs=" + VenderID.ToString() + "");
            ...
            if (ViewState["VendorQuotePOCart"] != null)
            {
                ...
                try
                {
                    ...
                    try
                    {
                        ...
                        if (Session["VendorPONotes"] != null)
                        {
                            if (Session["VendorPONotes"].ToString() == "")
                            {
                                strNotes += "N/A";
                            }
                            else
                            {
                                strNotes += System.Text.RegularExpressions.Regex.Replace(Session["VendorPONotes"].ToString().Trim().Replace("&nbsp;", "").Replace("\r\n\r\n", "\r\n"), @"<[^>]*>", String.Empty);
                            }
                        }
                        else
                        {
