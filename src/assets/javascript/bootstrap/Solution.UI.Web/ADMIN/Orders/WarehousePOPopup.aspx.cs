...
{
    public partial class WarehousePOPopup : BasePage
    {
        ...
        protected void btnAddToSelectionlist_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (grdProduct.Rows.Count > 0)
            {
                for (int i = 0; i < grdProduct.Rows.Count; i++)
                {
                    ...
                    {
                        if (string.IsNullOrEmpty(((TextBox)grdProduct.Rows[i].FindControl("txtQuantity")).Text.ToString()))
                        {
                            ((TextBox)grdProduct.Rows[i].FindControl("txtQuantity")).Focus();
                            Page.ClientScript.RegisterStartupScript(Page.GetType(), "", "alert('Please Enter Quantity!');", true);
                            return;
                        }
                    }
                }
                for (int i = 0; i < grdProduct.Rows.Count; i++)
                {
                    ...
                    if (chkSelect && !string.IsNullOrEmpty(SKUs.ToString()))
                    {
                        if (!string.IsNullOrEmpty(((TextBox)grdProduct.Rows[i].FindControl("txtQuantity")).Text.ToString()))
                        {
                            Quantity = Convert.ToInt32(((TextBox)grdProduct.Rows[i].FindControl("txtQuantity")).Text);
                            AddtoList(SKUs);
                            Cnt++;
                        }
                    }
        ...
        /// <summary>
        /// Add the Selected SKU into ViewState
        /// </summary>
        ...
        /// <summary>
        /// Remove data for OP of Warehouse
        /// </summary>
        /// <param name="lblSKU">string lblSKU</param>
