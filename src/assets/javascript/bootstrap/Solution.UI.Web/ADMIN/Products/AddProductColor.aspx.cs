...
{
    public partial class AddProductColor : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                ProductTempPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Color/Temp/");
                ProductIconPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Color/Icon/");
                ProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Color/Iphone/");
                ProductLargePath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Color/Large/");
                ProductMicroPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Color/Micro/");
        ...
        public void BindData(Int32 ID)
        {
            ...
            DsProduct = objPro.GetProductColorbyID(ID);

            if (DsProduct != null && DsProduct.Tables.Count > 0 && DsProduct.Tables[0].Rows.Count > 0)
            {
                txtColor.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["ColorName"]);
                ...
                #region Get Image
                ProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Color/Large/");
        ...
        {
            Int32 IsExists = Convert.ToInt32(CommonComponent.GetScalarCommonData("select isnull(count(1),0) as cCount from tb_Color where ColorName = '" + txtColor.Text.ToString().Trim().Replace("'", "''") + "' and isnull(Deleted,0) = 0"));
            if (IsExists > 0)
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "IsExists", "$(document).ready( function() {jAlert('Color name is already exists specify another Color name', 'Message','ContentPlaceHolder1_txtColor');});", true);
                txtColor.Focus();
                return;
            }
            else
            {
                ...
                int retColorID = 0;
                retColorID = ProductComponent.InsertUpdateProductColor(0, txtColor.Text.ToString().Trim(), strImageName, chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()));

                if (!string.IsNullOrEmpty(txtColor.Text.ToString().Trim()) && retColorID > 0)
                {
                    strImageName = RemoveSpecialCharacter(Convert.ToString(txtColor.Text.ToString()).ToCharArray()) + "_" + retColorID + ".jpg";
                    strImageName = strImageName.Replace(" ", "").Replace("&", "-");
                    SaveImage(strImageName);
                    CommonComponent.ExecuteCommonData("update tb_Color set ImgName = '" + strImageName.ToString().Trim().Replace("'", "''") + "' where ColorID = " + retColorID);
                }

                Response.Redirect("ProductColorList.aspx?Insert=true");
            }
        }
        ...
        public void UpdateProduct(int ColorID)
        {
            ProductComponent objProduct = new ProductComponent();

            Int32 ChkSKu = Convert.ToInt32(CommonComponent.GetScalarCommonData("select ISNULL(count(ColorName),0) as TotCnt From tb_Color Where ColorName='" + txtColor.Text.ToString().Trim() + "' And  ColorID <>" + ColorID + " AND ISNULL(Deleted,0)=0 "));
            if (ChkSKu > 0)
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "chkSku", "$(document).ready( function() {jAlert('Color with same name already exists specify another Color', 'Message','ContentPlaceHolder1_txtColor');});", true);
                txtColor.Focus();
                return;
            }
            else
            {
                ...
                try
                {
                    if (!string.IsNullOrEmpty(txtColor.Text.ToString().Trim()))
                    {
                       // strImageName = RemoveSpecialCharacter(Convert.ToString(txtColor.Text.ToString()).ToCharArray()) + "_" + ColorID + ".jpg";
                    ...
                    ProductComponent.InsertUpdateProductColor(ColorID, txtColor.Text.ToString().Trim(), strImageName, chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()));
                ...
                Response.Redirect("ProductColorList.aspx?Update=true");
            }
        }
        ...
        {
            DataSet dsIconWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "ColorIconWidth");
            DataSet dsIconHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "ColorIconHeight");
            ...
            DataSet dsMediumWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "ColorIphoneWidth");
            DataSet dsMediumHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "ColorIphoneHeight");
            ...
            DataSet dsMicroWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "ColorMicroWidth");
            DataSet dsMicroHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "ColorMicroHeight");
        ...
        protected void btnDelete_Click(object sender, ImageClickEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    ImgLarge.Src = "";
                    String strProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Color/Icon/");
        ...
        {
            Response.Redirect("ProductColorList.aspx");
        }
