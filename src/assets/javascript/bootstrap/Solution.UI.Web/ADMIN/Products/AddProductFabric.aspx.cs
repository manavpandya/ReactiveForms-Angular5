...
{
    public partial class AddProductFabric : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                ProductTempPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Fabric/Temp/");
                ProductIconPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Fabric/Icon/");
                ProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Fabric/Iphone/");
                ProductLargePath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Fabric/Large/");
                ProductMicroPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Fabric/Micro/");
        ...
        /// </summary>
        private void BindFabricGroupType()
        {
            ProductComponent objproduct = new ProductComponent();
            DataSet dsproduct = objproduct.GetProductFabricGroupTypeList(0, false, "", "");
            if (dsproduct != null && dsproduct.Tables.Count > 0 && dsproduct.Tables[0].Rows.Count > 0)
            {
                ddlFabricGroupType.DataSource = dsproduct;
                ddlFabricGroupType.DataTextField = "FabricGroupName";
                ddlFabricGroupType.DataValueField = "FabricGroupTypeId";
                ddlFabricGroupType.DataBind();
            }
        }
        ...
        public void BindData(Int32 ID)
        {
            ...
            DsProduct = objPro.GetProductFabricbyID(ID);

            if (DsProduct != null && DsProduct.Tables.Count > 0 && DsProduct.Tables[0].Rows.Count > 0)
            {
                txtFabric.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["FabricName"]);
                //txtDescription.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["FabricDescription"]);
                ...
                ddlFabricGroupType.Items.FindByValue(Convert.ToString(DsProduct.Tables[0].Rows[0]["FabricGroupType"])).Selected = true;
                hdnimage.Value = Convert.ToString(DsProduct.Tables[0].Rows[0]["ImgName"]);
                #region Get Image
                ProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Fabric/Large/");
        ...
        {
            Int32 IsExists = Convert.ToInt32(CommonComponent.GetScalarCommonData("select isnull(count(1),0) as cCount from tb_Fabric where FabricName = '" + txtFabric.Text.ToString().Trim().Replace("'", "''") + "' and isnull(Deleted,0) = 0"));
            if (IsExists > 0)
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "IsExists", "$(document).ready( function() {jAlert('Fabric name is already exists specify another Fabric name', 'Message','ContentPlaceHolder1_txtFabric');});", true);
                txtFabric.Focus();
                return;
            }
            else
            {
                string strImageName = "";
                int retColorID = 0;
                retColorID = ProductComponent.InsertUpdateProductFabric(0, txtFabric.Text.ToString().Trim(), Convert.ToInt32(ddlFabricGroupType.SelectedValue.ToString()), chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()), strImageName);
                if (!string.IsNullOrEmpty(txtFabric.Text.ToString().Trim()))
                {
                    strImageName = RemoveSpecialCharacter(Convert.ToString(txtFabric.Text.ToString()).ToCharArray()) + "_" + retColorID + ".jpg";
                    strImageName = strImageName.Replace(" ", "").Replace("&", "-");
                    SaveImage(strImageName);
                    CommonComponent.ExecuteCommonData("update tb_Fabric set ImgName = '" + strImageName.ToString().Trim().Replace("'", "''") + "' where FabricID = " + retColorID);
                }
                Response.Redirect("ProductFabricList.aspx?Insert=true");
            }
        }
        ...
        public void UpdateProduct(int FabricID)
        {
            ProductComponent objProduct = new ProductComponent();

            Int32 ChkSKu = Convert.ToInt32(CommonComponent.GetScalarCommonData("select ISNULL(count(FabricName),0) as TotCnt From tb_Fabric Where FabricName='" + txtFabric.Text.ToString().Trim() + "' And  FabricID <>" + FabricID + " AND ISNULL(Deleted,0)=0 "));
            if (ChkSKu > 0)
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "chkSku", "$(document).ready( function() {jAlert('Fabric with same name already exists specify another Fabric', 'Message','ContentPlaceHolder1_txtFabric');});", true);
                txtFabric.Focus();
                return;
            }
            else
            {
                ...
                try
                {
                    if (!string.IsNullOrEmpty(txtFabric.Text.ToString().Trim()))
                    {
                        //strImageName = RemoveSpecialCharacter(Convert.ToString(txtFabric.Text.ToString()).ToCharArray()) + "_" + FabricID + ".jpg";
                    ...
                    ProductComponent.InsertUpdateProductFabric(FabricID, txtFabric.Text.ToString().Trim(), Convert.ToInt32(ddlFabricGroupType.SelectedValue.ToString()), chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()), strImageName);
                ...
                Response.Redirect("ProductFabricList.aspx?Update=true");
            }
        }
        ...
        {
            DataSet dsIconWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FabricIconWidth");
            DataSet dsIconHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FabricIconHeight");
            ...
            DataSet dsMediumWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FabricIphoneWidth");
            DataSet dsMediumHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FabricIphoneHeight");
            ...
            DataSet dsMicroWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FabricMicroWidth");
            DataSet dsMicroHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FabricMicroHeight");
        ...
        protected void btnDelete_Click(object sender, ImageClickEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    ImgLarge.Src = "";
                    String strProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Fabric/Icon/");
        ...
        {
            Response.Redirect("ProductFabricList.aspx");
        }
