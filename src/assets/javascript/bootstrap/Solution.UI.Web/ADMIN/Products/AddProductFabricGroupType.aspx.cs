...
{
    public partial class AddProductFabricGroupType : BasePage
    {
        ...
        static public int FabricGroupTypeID = 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                ProductTempPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "FabricGroupType/Temp/");
                ProductIconPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "FabricGroupType/Icon/");
                ProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "FabricGroupType/Iphone/");
                ProductLargePath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "FabricGroupType/Large/");
                ProductMicroPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "FabricGroupType/Micro/");
        ...
        public void BindData(Int32 ID)
        {
            ...
            DsProduct = objPro.GetProductFabricGroupTypebyID(ID);

            if (DsProduct != null && DsProduct.Tables.Count > 0 && DsProduct.Tables[0].Rows.Count > 0)
            {
                txtFabricGroupName.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["FabricGroupName"]);
        ...
        {
            string strImageName = "";
            if (!string.IsNullOrEmpty(txtFabricGroupName.Text.ToString().Trim()))
            {
                strImageName = txtFabricGroupName.Text.ToString() + "_" + FabricGroupTypeID + ".jpg";
                strImageName = strImageName.Replace(" ", "").Replace("&", "-");
                SaveImage(strImageName);
            }

            ProductComponent.InsertUpdateProductFabricGroupType(0, txtFabricGroupName.Text.ToString().Trim(), chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()));

            Response.Redirect("ProductFabricGroupTypeList.aspx?Insert=true");
        }
        ...
        public void UpdateProduct(int FabricGroupTypeID)
        {
            ProductComponent objProduct = new ProductComponent();

            Int32 ChkSKu = Convert.ToInt32(CommonComponent.GetScalarCommonData("select ISNULL(count(FabricGroupName),0) as TotCnt From tb_FabricGroupType Where FabricGroupName='" + txtFabricGroupName.Text.ToString().Trim() + "' And  FabricGroupTypeID <>" + FabricGroupTypeID + " AND ISNULL(Deleted,0)=0 "));
            if (ChkSKu > 0)
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "chkSku", "$(document).ready( function() {jAlert('Fabric Group Name with same name already exists specify another Fabric Group Name', 'Message','ContentPlaceHolder1_txtFabricGroupName');});", true);
                txtFabricGroupName.Focus();
                return;
            }
            else
            {
                ...
                try
                {
                    ...
                    ProductComponent.InsertUpdateProductFabricGroupType(FabricGroupTypeID, txtFabricGroupName.Text.ToString().Trim(), chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()));
                ...
                Response.Redirect("ProductFabricGroupTypeList.aspx?Update=true");
            }
        }
        ...
        {
            DataSet dsIconWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FabricGroupTypeIconWidth");
            DataSet dsIconHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FabricGroupTypeIconHeight");
            ...
            DataSet dsMediumWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FabricGroupTypeIphoneWidth");
            DataSet dsMediumHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FabricGroupTypeIphoneHeight");
            ...
            DataSet dsMicroWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FabricGroupTypeMicroWidth");
            DataSet dsMicroHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FabricGroupTypeMicroHeight");
        ...
        protected void btnDelete_Click(object sender, ImageClickEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    ImgLarge.Src = "";
                    String strProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "FabricGroupType/Icon/");
        ...
        {
            Response.Redirect("ProductFabricGroupTypeList.aspx");
        }
