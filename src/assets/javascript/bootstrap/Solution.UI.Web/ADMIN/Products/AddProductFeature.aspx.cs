...
{
    public partial class AddProductFeature : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                ProductTempPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Feature/Temp/");
                ProductIconPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Feature/Icon/");
                ProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Feature/Iphone/");
                ProductLargePath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Feature/Large/");
                ProductMicroPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Feature/Micro/");

                BindSize();
                BindFeatureGroupType();
        ...
        /// </summary>
        private void BindFeatureGroupType()
        {
            ProductComponent objproduct = new ProductComponent();
            DataSet dsproduct = objproduct.GetProductFeatureGroupTypeList(0, false, "", "");
            if (dsproduct != null && dsproduct.Tables.Count > 0 && dsproduct.Tables[0].Rows.Count > 0)
            {
                ddlFeatureGroupType.DataSource = dsproduct;
                ddlFeatureGroupType.DataTextField = "FeatureGroupName";
                ddlFeatureGroupType.DataValueField = "FeatureGroupTypeId";
                ddlFeatureGroupType.DataBind();
            }
        }
        ...
        public void BindData(Int32 ID)
        {
            ...
            DsProduct = objPro.GetProductFeaturebyID(ID);

            if (DsProduct != null && DsProduct.Tables.Count > 0 && DsProduct.Tables[0].Rows.Count > 0)
            {
                txtFeature.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["FeatureName"]);
                //txtDescription.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["FeatureDescription"]);
                ...
                ddlFeatureGroupType.Items.FindByValue(Convert.ToString(DsProduct.Tables[0].Rows[0]["FeatureGroupType"])).Selected = true;
                hdnimage.Value = Convert.ToString(DsProduct.Tables[0].Rows[0]["ImgName"]);
                #region Get Image
                ProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Feature/Large/");
        ...
        {
            Int32 IsExists = Convert.ToInt32(CommonComponent.GetScalarCommonData("select isnull(count(1),0) as cCount from tb_Feature where FabricName = '" + txtFeature.Text.ToString().Trim().Replace("'", "''") + "' and isnull(Deleted,0) = 0"));
            if (IsExists > 0)
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "IsExists", "$(document).ready( function() {jAlert('Fabric name is already exists specify another Fabric name', 'Message','ContentPlaceHolder1_txtFeature');});", true);
                txtFeature.Focus();
                return;
            }
            else
            {
                ...
                int retColorID = 0;
                retColorID = ProductComponent.InsertUpdateProductFeature(0, txtFeature.Text.ToString().Trim(), Convert.ToInt32(ddlFeatureGroupType.SelectedValue.ToString()), chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()), strImageName);

                if (!string.IsNullOrEmpty(txtFeature.Text.ToString().Trim()))
                {
                    strImageName = RemoveSpecialCharacter(Convert.ToString(txtFeature.Text.ToString()).ToCharArray()) + "_" + retColorID + ".jpg";
                    strImageName = strImageName.Replace(" ", "").Replace("&", "-");
                    SaveImage(strImageName);
                    CommonComponent.ExecuteCommonData("update tb_Feature set ImgName = '" + strImageName.ToString().Trim().Replace("'", "''") + "' where FeatureID = " + retColorID);
                }
                Response.Redirect("ProductFeatureList.aspx?Insert=true");
            }
        }
        ...
        public void UpdateProduct(int FeatureID)
        {
            ProductComponent objProduct = new ProductComponent();

            Int32 ChkSKu = Convert.ToInt32(CommonComponent.GetScalarCommonData("select ISNULL(count(FeatureName),0) as TotCnt From tb_Feature Where FeatureName='" + txtFeature.Text.ToString().Trim() + "' And  FeatureID <>" + FeatureID + " AND ISNULL(Deleted,0)=0 "));
            if (ChkSKu > 0)
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "chkSku", "$(document).ready( function() {jAlert('Feature with same name already exists specify another Feature', 'Message','ContentPlaceHolder1_txtFeature');});", true);
                txtFeature.Focus();
                return;
            }
            else
            {
                ...
                try
                {
                    if (!string.IsNullOrEmpty(txtFeature.Text.ToString().Trim()))
                    {
                       // strImageName = RemoveSpecialCharacter(Convert.ToString(txtFeature.Text.ToString()).ToCharArray()) + "_" + FeatureID + ".jpg";
                    ...
                    ProductComponent.InsertUpdateProductFeature(FeatureID, txtFeature.Text.ToString().Trim(), Convert.ToInt32(ddlFeatureGroupType.SelectedValue.ToString()), chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()), strImageName);
                ...
                Response.Redirect("ProductFeatureList.aspx?Update=true");
            }
        }
        ...
        {
            DataSet dsIconWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FeatureIconWidth");
            DataSet dsIconHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FeatureIconHeight");
            ...
            DataSet dsMediumWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FeatureIphoneWidth");
            DataSet dsMediumHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FeatureIphoneHeight");
            ...
            DataSet dsMicroWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FeatureMicroWidth");
            DataSet dsMicroHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FeatureMicroHeight");
        ...
        protected void btnDelete_Click(object sender, ImageClickEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    ImgLarge.Src = "";
                    String strProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Feature/Icon/");
        ...
        {
            Response.Redirect("ProductFeatureList.aspx");
        }
