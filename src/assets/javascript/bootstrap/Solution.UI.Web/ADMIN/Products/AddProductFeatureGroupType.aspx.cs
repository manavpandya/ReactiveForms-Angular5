...
{
    public partial class AddProductFeatureGroupType : BasePage
    {
        ...
        static public int FeatureGroupTypeID = 0;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                ProductTempPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "FeatureGroupType/Temp/");
                ProductIconPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "FeatureGroupType/Icon/");
                ProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "FeatureGroupType/Iphone/");
                ProductLargePath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "FeatureGroupType/Large/");
                ProductMicroPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "FeatureGroupType/Micro/");
        ...
        public void BindData(Int32 ID)
        {
            ...
            DsProduct = objPro.GetProductFeatureGroupTypebyID(ID);

            if (DsProduct != null && DsProduct.Tables.Count > 0 && DsProduct.Tables[0].Rows.Count > 0)
            {
                txtFeatureGroupName.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["FeatureGroupName"]);
        ...
        {
            string strImageName = "";
            if (!string.IsNullOrEmpty(txtFeatureGroupName.Text.ToString().Trim()))
            {
                strImageName = txtFeatureGroupName.Text.ToString() + "_" + FeatureGroupTypeID + ".jpg";
                strImageName = strImageName.Replace(" ", "").Replace("&", "-");
                SaveImage(strImageName);
            }

            ProductComponent.InsertUpdateProductFeatureGroupType(0, txtFeatureGroupName.Text.ToString().Trim(), chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()));

            Response.Redirect("ProductFeatureGroupTypeList.aspx?Insert=true");
        }
        ...
        public void UpdateProduct(int FeatureGroupTypeID)
        {
            ProductComponent objProduct = new ProductComponent();

            Int32 ChkSKu = Convert.ToInt32(CommonComponent.GetScalarCommonData("select ISNULL(count(FeatureGroupName),0) as TotCnt From tb_FeatureGroupType Where FeatureGroupName='" + txtFeatureGroupName.Text.ToString().Trim() + "' And  FeatureGroupTypeID <>" + FeatureGroupTypeID + " AND ISNULL(Deleted,0)=0 "));
            if (ChkSKu > 0)
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "chkSku", "$(document).ready( function() {jAlert('Feature Group Name with same name already exists specify another Feature Group Name', 'Message','ContentPlaceHolder1_txtFeatureGroupName');});", true);
                txtFeatureGroupName.Focus();
                return;
            }
            else
            {
                ...
                try
                {
                    ...
                    ProductComponent.InsertUpdateProductFeatureGroupType(FeatureGroupTypeID, txtFeatureGroupName.Text.ToString().Trim(), chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()));
                ...
                Response.Redirect("ProductFeatureGroupTypeList.aspx?Update=true");
            }
        }
        ...
        {
            DataSet dsIconWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FeatureGroupTypeIconWidth");
            DataSet dsIconHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FeatureGroupTypeIconHeight");
            ...
            DataSet dsMediumWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FeatureGroupTypeIphoneWidth");
            DataSet dsMediumHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FeatureGroupTypeIphoneHeight");
            ...
            DataSet dsMicroWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FeatureGroupTypeMicroWidth");
            DataSet dsMicroHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "FeatureGroupTypeMicroHeight");
        ...
        protected void btnDelete_Click(object sender, ImageClickEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    ImgLarge.Src = "";
                    String strProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "FeatureGroupType/Icon/");
        ...
        {
            Response.Redirect("ProductFeatureGroupTypeList.aspx");
        }
