...
{
    public partial class AddProductHeader : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                ProductTempPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Header/Temp/");
                ProductIconPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Header/Icon/");
                ProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Header/Iphone/");
                ProductLargePath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Header/Large/");
                ProductMicroPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Header/Micro/");
        ...
        public void BindData(Int32 ID)
        {
            ...
            DsProduct = objPro.GetProductHeaderbyID(ID);

            if (DsProduct != null && DsProduct.Tables.Count > 0 && DsProduct.Tables[0].Rows.Count > 0)
            {
                txtHeader.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["HeaderName"]);
                txtDescription.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["HeaderDescription"]);
                ...
                #region Get Image
                ProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Header/Large/");
        ...
        {
            Int32 IsExists = Convert.ToInt32(CommonComponent.GetScalarCommonData("select isnull(count(1),0) as cCount from tb_Header where HeaderName = '" + txtHeader.Text.ToString().Trim().Replace("'", "''") + "' and isnull(Deleted,0) = 0"));
            if (IsExists > 0)
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "IsExists", "$(document).ready( function() {jAlert('Header name is already exists specify another Header name', 'Message','ContentPlaceHolder1_txtHeader');});", true);
                txtHeader.Focus();
                return;
            }
            else
            {
                ...
                int retColorID = 0;
                retColorID = ProductComponent.InsertUpdateProductHeader(0, txtHeader.Text.ToString().Trim(), txtDescription.Text.ToString().Trim(), chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()), strImageName);

                if (!string.IsNullOrEmpty(txtHeader.Text.ToString().Trim()))
                {
                    strImageName = RemoveSpecialCharacter(Convert.ToString(txtHeader.Text.ToString()).ToCharArray()) + "_" + retColorID + ".jpg";
                    strImageName = strImageName.Replace(" ", "").Replace("&", "-");
                    SaveImage(strImageName);
                    CommonComponent.ExecuteCommonData("update tb_Header set ImageName = '" + strImageName.ToString().Trim().Replace("'", "''") + "' where HeaderID = " + retColorID);
                }

                Response.Redirect("ProductHeaderList.aspx?Insert=true");
            }
        }
        ...
        public void UpdateProduct(int HeaderID)
        {
            ProductComponent objProduct = new ProductComponent();

            Int32 ChkSKu = Convert.ToInt32(CommonComponent.GetScalarCommonData("select ISNULL(count(HeaderName),0) as TotCnt From tb_Header Where HeaderName='" + txtHeader.Text.ToString().Trim() + "' And   HeaderID <>" + HeaderID + " AND ISNULL(Deleted,0)=0 "));
            if (ChkSKu > 0)
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "chkSku", "$(document).ready( function() {jAlert('Header with same name already exists specify another Header', 'Message','ContentPlaceHolder1_txtHeader');});", true);
                txtHeader.Focus();
                return;
            }
            else
            {
                ...
                try
                {
                    if (!string.IsNullOrEmpty(txtHeader.Text.ToString().Trim()))
                    {
                        //strImageName = RemoveSpecialCharacter(Convert.ToString(txtHeader.Text.ToString()).ToCharArray()) + "_" + HeaderID + ".jpg";
                    ...
                    ProductComponent.InsertUpdateProductHeader(HeaderID, txtHeader.Text.ToString().Trim(), txtDescription.Text.ToString().Trim(), chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()), strImageName);
                ...
                Response.Redirect("ProductHeaderList.aspx?Update=true");
            }
        }
        ...
        {
            DataSet dsIconWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "HeaderIconWidth");
            DataSet dsIconHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "HeaderIconHeight");
            ...
            DataSet dsMediumWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "HeaderIphoneWidth");
            DataSet dsMediumHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "HeaderIphoneHeight");
            ...
            DataSet dsMicroWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "HeaderMicroWidth");
            DataSet dsMicroHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "HeaderMicroHeight");
        ...
        protected void btnDelete_Click(object sender, ImageClickEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    ImgLarge.Src = "";
                    String strProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Header/Icon/");
        ...
        {
            Response.Redirect("ProductHeaderList.aspx");
        }
