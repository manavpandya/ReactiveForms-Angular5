...
{
    public partial class AddProductRoom : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                ProductTempPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Room/Temp/");
                ProductIconPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Room/Icon/");
                ProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Room/Iphone/");
                ProductLargePath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Room/Large/");
                ProductMicroPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Room/Micro/");
        ...
        public void BindData(Int32 ID)
        {
            ...
            DsProduct = objPro.GetProductRoombyID(ID);

            if (DsProduct != null && DsProduct.Tables.Count > 0 && DsProduct.Tables[0].Rows.Count > 0)
            {
                txtRoom.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["RoomName"]);
                txtDescription.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["RoomDescription"]);
                ...
                #region Get Image
                ProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Room/Large/");
        ...
        {
            Int32 IsExists = Convert.ToInt32(CommonComponent.GetScalarCommonData("select isnull(count(1),0) as cCount from tb_Room where RoomName = '" + txtRoom.Text.ToString().Trim().Replace("'", "''") + "' and isnull(Deleted,0) = 0"));
            if (IsExists > 0)
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "IsExists", "$(document).ready( function() {jAlert('Room name is already exists specify another Room name', 'Message','ContentPlaceHolder1_txtRoom');});", true);
                txtRoom.Focus();
                return;
            }
            else
            {
                string strImageName = "";
                int retColorID = 0;
                retColorID = ProductComponent.InsertUpdateProductRoom(0, txtRoom.Text.ToString().Trim(), txtDescription.Text.ToString().Trim(), chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()), strImageName);
                if (!string.IsNullOrEmpty(txtRoom.Text.ToString().Trim()))
                {
                    strImageName = RemoveSpecialCharacter(Convert.ToString(txtRoom.Text.ToString()).ToCharArray()) + "_" + retColorID + ".jpg";
                    strImageName = strImageName.Replace(" ", "").Replace("&", "-");
                    SaveImage(strImageName);
                    CommonComponent.ExecuteCommonData("update tb_Room set ImgName = '" + strImageName.ToString().Trim().Replace("'", "''") + "' where RoomID = " + retColorID);
                }

                Response.Redirect("ProductRoomList.aspx?Insert=true");
            }
        }
        ...
        public void UpdateProduct(int RoomID)
        {
            ProductComponent objProduct = new ProductComponent();

            Int32 ChkSKu = Convert.ToInt32(CommonComponent.GetScalarCommonData("select ISNULL(count(RoomName),0) as TotCnt From tb_Room Where RoomName='" + txtRoom.Text.ToString().Trim() + "' And  RoomID <>" + RoomID + " AND ISNULL(Deleted,0)=0 "));
            if (ChkSKu > 0)
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "chkSku", "$(document).ready( function() {jAlert('Room with same name already exists specify another Room', 'Message','ContentPlaceHolder1_txtRoom');});", true);
                txtRoom.Focus();
                return;
            }
            else
            {
                ...
                try
                {
                    if (!string.IsNullOrEmpty(txtRoom.Text.ToString().Trim()))
                    {
                        //strImageName = RemoveSpecialCharacter(Convert.ToString(txtRoom.Text.ToString()).ToCharArray()) + "_" + RoomID + ".jpg";
                    ...
                    ProductComponent.InsertUpdateProductRoom(RoomID, txtRoom.Text.ToString().Trim(), txtDescription.Text.ToString().Trim(), chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()), strImageName);
                ...
                Response.Redirect("ProductRoomList.aspx?Update=true");
            }
        }
        ...
        {
            DataSet dsIconWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "RoomIconWidth");
            DataSet dsIconHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "RoomIconHeight");
            ...
            DataSet dsMediumWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "RoomIphoneWidth");
            DataSet dsMediumHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "RoomIphoneHeight");
            ...
            DataSet dsMicroWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "RoomMicroWidth");
            DataSet dsMicroHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "RoomMicroHeight");
        ...
        protected void btnDelete_Click(object sender, ImageClickEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    ImgLarge.Src = "";
                    String strProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Room/Icon/");
        ...
        {
            Response.Redirect("ProductRoomList.aspx");
        }
