...
{
    public partial class AddProductStyle : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                ProductTempPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Style/Temp/");
                ProductIconPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Style/Icon/");
                ProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Style/Iphone/");
                ProductLargePath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Style/Large/");
                ProductMicroPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Style/Micro/");
        ...
        public void BindData(Int32 ID)
        {
            ...
            DsProduct = objPro.GetProductStylebyID(ID);

            if (DsProduct != null && DsProduct.Tables.Count > 0 && DsProduct.Tables[0].Rows.Count > 0)
            {
                txtStyle.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["StyleName"]);
                txtDescription.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["StyleDescription"]);
                ...
                #region Get Image
                ProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Style/Large/");
        ...
        {
            Int32 IsExists = Convert.ToInt32(CommonComponent.GetScalarCommonData("select isnull(count(1),0) as cCount from tb_Style where StyleName = '" + txtStyle.Text.ToString().Trim().Replace("'", "''") + "' and isnull(Deleted,0) = 0"));
            if (IsExists > 0)
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "IsExists", "$(document).ready( function() {jAlert('Style name is already exists specify another Style name', 'Message','ContentPlaceHolder1_txtStyle');});", true);
                txtStyle.Focus();
                return;
            }
            else
            {
                ...
                int retColorID = 0;
                retColorID = ProductComponent.InsertUpdateProductStyle(0, txtStyle.Text.ToString().Trim(), txtDescription.Text.ToString().Trim(), chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()), strImageName);

                if (!string.IsNullOrEmpty(txtStyle.Text.ToString().Trim()))
                {
                    strImageName = RemoveSpecialCharacter(Convert.ToString(txtStyle.Text.ToString()).ToCharArray()) + "_" + retColorID + ".jpg";
                    strImageName = strImageName.Replace(" ", "").Replace("&", "-");
                    SaveImage(strImageName);
                    CommonComponent.ExecuteCommonData("update tb_Style set ImgName = '" + strImageName.ToString().Trim().Replace("'", "''") + "' where StyleID = " + retColorID);
                }

                Response.Redirect("ProductStyleList.aspx?Insert=true");
            }
        }
        ...
        public void UpdateProduct(int StyleID)
        {
            ProductComponent objProduct = new ProductComponent();

            Int32 ChkSKu = Convert.ToInt32(CommonComponent.GetScalarCommonData("select ISNULL(count(StyleName),0) as TotCnt From tb_Style Where StyleName='" + txtStyle.Text.ToString().Trim() + "' And  StyleID <>" + StyleID + " AND ISNULL(Deleted,0)=0 "));
            if (ChkSKu > 0)
            {
                Page.ClientScript.RegisterStartupScript(Page.GetType(), "chkSku", "$(document).ready( function() {jAlert('Style with same name already exists specify another Style', 'Message','ContentPlaceHolder1_txtStyle');});", true);
                txtStyle.Focus();
                return;
            }
            else
            {
                ...
                try
                {
                    if (!string.IsNullOrEmpty(txtStyle.Text.ToString().Trim()))
                    {
                       // strImageName = RemoveSpecialCharacter(Convert.ToString(txtStyle.Text.ToString()).ToCharArray()) + "_" + StyleID + ".jpg";
                    ...
                    ProductComponent.InsertUpdateProductStyle(StyleID, txtStyle.Text.ToString().Trim(), txtDescription.Text.ToString().Trim(), chkIsActive.Checked ? true : false, false, Convert.ToInt32(txtDisplayOrder.Text.ToString()), strImageName);
                ...
                Response.Redirect("ProductStyleList.aspx?Update=true");
            }
        }
        ...
        {
            DataSet dsIconWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "StyleIconWidth");
            DataSet dsIconHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "StyleIconHeight");
            ...
            DataSet dsMediumWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "StyleIphoneWidth");
            DataSet dsMediumHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "StyleIphoneHeight");
            ...
            DataSet dsMicroWidth = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "StyleMicroWidth");
            DataSet dsMicroHeight = objConfiguration.GetImageSizeByType(AppConfig.StoreID, "StyleMicroHeight");
        ...
        protected void btnDelete_Click(object sender, ImageClickEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    ImgLarge.Src = "";
                    String strProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Style/Icon/");
        ...
        {
            Response.Redirect("ProductStyleList.aspx");
        }
