...
{
    public partial class EditShadeMarkup : BasePage
    {
        string strHTMl = "";
        ...
        private void GetShadeMarkupPrice()
        {
            decimal suggestedretail = 0;
            suggestedretail = Convert.ToDecimal(CommonComponent.GetScalarCommonData("select Configvalue from tb_appconfig where configname='ShadeMarkup' and storeid=1"));
            txtShadeMarkUp.Text = suggestedretail.ToString();
        }


        protected void btnSaveValue_Click(object sender, ImageClickEventArgs e)
        {
            CommonComponent.ExecuteCommonData("update tb_appconfig set configvalue='" + (txtShadeMarkUp.Text) + "' where configname='ShadeMarkup' and storeid=1");
            DisplayShadeCalc();
            Page.ClientScript.RegisterStartupScript(btnSaveValue.GetType(), "@closemsg", "window.opener.document.getElementById('ContentPlaceHolder1_txtMarkup').value='" + txtShadeMarkUp.Text + "';window.opener.document.getElementById('ContentPlaceHolder1_divshadecalculator').innerHTML='" + strHTMl + "';window.close();", true);
        }
