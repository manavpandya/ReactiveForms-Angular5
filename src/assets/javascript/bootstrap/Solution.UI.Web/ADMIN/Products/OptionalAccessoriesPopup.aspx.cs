...
{
    public partial class OptionalAccessoriesPopup : BasePage
    {
        ...
        protected void BindSearchData()
        {
            ...
            if (!string.IsNullOrEmpty(txtSearch.Text.ToString().Trim()))
            {
                WhrClus += " Where " + ddlSearchby.SelectedValue.ToString() + " like '%" + txtSearch.Text.ToString().Trim() + "%' And (ISNULL(tb_Product.Deleted,0) != 1) And (tb_Product.Active = 1) and (tb_Product.StoreId = " + Request.QueryString["StoreId"] + ")" + "and Productid <>" + Request.QueryString["ProductID"];
            }
            else
                WhrClus = "where Productid <>" + Request.QueryString["ProductID"] + "And (ISNULL(tb_Product.Deleted,0) != 1) And (tb_Product.Active = 1) AND (tb_Product.StoreId = " + Request.QueryString["StoreId"] + ")";
        ...
        /// <summary>
        ///  Add Selected Item Button Click Event
        /// </summary>
        /// <param name="sender">object sender</param>
        /// <param name="e">EventArgs e</param>
