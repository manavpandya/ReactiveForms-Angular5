...
{
    public partial class ProductFabricSKU : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["StoreID"] != null && Request.QueryString["clientid"].ToString() != null && !string.IsNullOrEmpty(Request.QueryString["clientid"].ToString()))
                {
                    ...
                    {
                        ViewState["SelectedSKUs"] = Request.QueryString["vname"].ToString();
                        BindSelectedData();
                    }
                    else
                    {
        ...
        protected void BindSearchData()
        {
            ...
            if (!string.IsNullOrEmpty(txtSearch.Text.ToString().Trim()))
            {
                WhrClus += " Where " + ddlSearchby.SelectedValue.ToString() + " like '%" + txtSearch.Text.ToString().Trim() + "%' And (ISNULL(tb_Product.Deleted,0) != 1) And (tb_Product.Active = 1) and ProductID <>" + Convert.ToString(Request.QueryString["ProductID"]) + " AND (tb_Product.StoreId = " + Request.QueryString["StoreId"] + ")";
            }
            else
                WhrClus = " Where ProductID <>" + Convert.ToString(Request.QueryString["ProductID"]) + " And (ISNULL(tb_Product.Deleted,0) != 1) And (tb_Product.Active = 1) and (tb_Product.StoreId = " + Request.QueryString["StoreId"] + ")";
            //WhrClus = "";
        ...
        private void AddtoList(string lblSKU)
        {
            try
            {
                ...
                ViewState["SelectedSKUs"] = lblSKU;
                if (Request.QueryString["clientid"].ToString().IndexOf("arelatedsku1") > -1)
                {
                    Page.ClientScript.RegisterClientScriptBlock(btnAddSelectedItems.GetType(), "@closemsg", "window.opener.document.getElementById('" + Request.QueryString["clientid"].ToString() + "').innerHTML = '" + lblSKU + "';window.opener.document.getElementById('ContentPlaceHolder1_hdnrelatedsku1').value = '" + lblSKU + "';", true);
                }
                else
                {
                    Page.ClientScript.RegisterClientScriptBlock(btnAddSelectedItems.GetType(), "@closemsg", "window.opener.document.getElementById('" + Request.QueryString["clientid"].ToString() + "').innerHTML = '" + lblSKU + "';window.opener.document.getElementById('ContentPlaceHolder1_hdnrelatedsku').value = '" + lblSKU + "';", true);
                }
            }
            catch { }
        }
        ...
        private void BindSelectedData()
        {
            ...
            if (!string.IsNullOrEmpty(SKUs))
            {
                ...
                string sql = "exec [usp_GetProductsUsingSkus] " + Request.QueryString["StoreId"].ToString() + ", '" + SKUs + "'";
        ...
        private void Removedata(string lblSKU)
        {
            try
            {
                ...
                ViewState["SelectedSKUs"] = "";
                if (ViewState["SelectedSKUs"].ToString() == "")
                {
                    ViewState["SelectedSKUs"] = "Select SKU";
                }
                //Page.ClientScript.RegisterClientScriptBlock(btnAddSelectedItems.GetType(), "@closemsg", "window.opener.document.getElementById('" + Request.QueryString["clientid"].ToString() + "').innerHTML = '" + skus + "';window.opener.document.getElementById('" + Request.QueryString["clientid"].ToString().Replace("arelatedsku", "hdnrelatedsku") + "').value = '" + skus + "';window.close();", true);

                if (Request.QueryString["clientid"].ToString().IndexOf("arelatedsku1") > -1)
                {
                    Page.ClientScript.RegisterClientScriptBlock(btnAddSelectedItems.GetType(), "@closemsg", "window.opener.document.getElementById('" + Request.QueryString["clientid"].ToString() + "').innerHTML = '" + ViewState["SelectedSKUs"].ToString() + "';window.opener.document.getElementById('ContentPlaceHolder1_hdnrelatedsku1').value = '" + ViewState["SelectedSKUs"].ToString() + "';", true);
                }
                else
                {
                    Page.ClientScript.RegisterClientScriptBlock(btnAddSelectedItems.GetType(), "@closemsg", "window.opener.document.getElementById('" + Request.QueryString["clientid"].ToString() + "').innerHTML = '" + ViewState["SelectedSKUs"].ToString() + "';window.opener.document.getElementById('ContentPlaceHolder1_hdnrelatedsku').value = '" + ViewState["SelectedSKUs"].ToString() + "';", true);
                }
                BindSelectedData();
            ...
            if (grdSelected.Rows.Count > 0)
            {
                try
                {
                    ...
                    { 
                        ...
                        if (Request.QueryString["clientid"].ToString().IndexOf("arelatedsku1") > -1)
                        {
                            Page.ClientScript.RegisterClientScriptBlock(btnAddSelectedItems.GetType(), "@closemsg", "window.opener.document.getElementById('" + Request.QueryString["clientid"].ToString() + "').innerHTML = '" + skus + "';window.opener.document.getElementById('ContentPlaceHolder1_hdnrelatedsku1').value = '" + skus + "';", true);
                        }
                        else
                        {
                            Page.ClientScript.RegisterClientScriptBlock(btnAddSelectedItems.GetType(), "@closemsg", "window.opener.document.getElementById('" + Request.QueryString["clientid"].ToString() + "').innerHTML = '" + skus + "';window.opener.document.getElementById('ContentPlaceHolder1_hdnrelatedsku').value = '" + skus + "';", true);
                        }
                    }
                }
                catch { }
        ...
        protected void grdProduct_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                CheckBox chknew = (CheckBox)e.Row.FindControl("chkSelect");
                chknew.Attributes.Add("onchange", "unselectcheckbox('" + chknew.ClientID.ToString() + "');");
                chknew.Attributes.Add("onclick", "unselectcheckbox('" + chknew.ClientID.ToString() + "');");
            }
        }
    }
}