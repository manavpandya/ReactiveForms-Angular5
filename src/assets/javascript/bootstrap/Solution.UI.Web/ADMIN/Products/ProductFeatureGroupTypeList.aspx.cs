...
{
    public partial class ProductFeatureGroupTypeList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    Page.ClientScript.RegisterStartupScript(Page.GetType(), "msg", "$(document).ready( function() {jAlert('Product Feature Group Type inserted successfully.', 'Message');});", true);

                }
                else if (Request.QueryString["Update"] != null)
                {
                    Page.ClientScript.RegisterStartupScript(Page.GetType(), "msg", "$(document).ready( function() {jAlert('Product Feature Group Type updated successfully.', 'Message');});", true);
                }
        ...
        private void binddata()
        {
            ...
            dsproduct = objproduct.GetProductFeatureGroupTypeList(0, Issearch, SearchBy, SearchValue);
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow r in gvProductSize.Rows)
            {
                ...
                Label lb = (Label)r.FindControl("lblFeatureGroupTypeID");
                int ID = Convert.ToInt32(lb.Text.ToString());
                if (chk.Checked)
                {
                    count++;
                    indx = ProductComponent.DeleteProductFeatureGroupType(Convert.ToInt32(ID));
                }
            }
        ...
        protected void gvProductSize_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "edit")
            {
                try
                {
                    int FeatureGroupTypeID = Convert.ToInt32(e.CommandArgument);
                    Response.Redirect("AddProductFeatureGroupType.aspx?Mode=Edit&ID=" + FeatureGroupTypeID);
                }
                catch
                { }
            }
