...
{
    public partial class ProductOverStock : BasePage
    {
        ...
        /// <summary>
        /// Bind Product Type Delivery By StoreID
        /// </summary>
        ...
        public void BindData(Int32 ID, Int32 StoreID)
        {
            ...
            if (DsProduct != null && DsProduct.Tables.Count > 0 && DsProduct.Tables[0].Rows.Count > 0)
            {
                ...
                txtOptionSKU.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["OptionSku"]);
                if (string.IsNullOrEmpty(DsProduct.Tables[0].Rows[0]["ShortName"].ToString()))
                {
                    txtShortName.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["Name"].ToString());
                }
                else
                {
                    txtShortName.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["ShortName"]);
                }
                txtPartNumber.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["PartNumber"]);
                ddlProvider.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["WarrantyProvider"]);
                txtWarrantyDescription.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["WarrantyDescription"]);
                txtWarrantyContactNumber.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["WarrantyContactPhoneNumber"]);
                txtMaterials.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["Materials"]);
                txtSummary.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["ProductSummary"]);
                txtSourceZipCode.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["SourceZipCode"]);
                txtBrand.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["Brand"]);
                if (string.IsNullOrEmpty(DsProduct.Tables[0].Rows[0]["ProductCondition"].ToString()) || DsProduct.Tables[0].Rows[0]["ProductCondition"].ToString().ToLower() == "select condition")
                {
                    ddlProductCondition.SelectedIndex = 1;
                }
                else
                {
                    try
                    {
                        ddlProductCondition.SelectedValue = Convert.ToString(DsProduct.Tables[0].Rows[0]["ProductCondition"]);
                    }
                    catch (Exception ex)
                    {
                        ddlProductCondition.SelectedIndex = 1;
        ...
        public void InsertProduct(int StoreID)
        {
            ...
            else
            {
                ...
                if (ProductID > 0)
                {
                    ...
                    {
                        Response.Redirect("ProductList.aspx?Insert=true&StoreID=" + ((Request.QueryString["StoreID"] != null) ? Convert.ToString(Request.QueryString["StoreID"]) : ""));
                        //System.Web.UI.ScriptManager.RegisterStartupScript(this, this.GetType(), "msg", "jAlert('Another UPC with same SKU already exists in another Store, UPC for this SKU will be updated with that UPC!');window.location ='ProductList.aspx?Insert=true&StoreID=" + ((Request.QueryString["StoreID"] != null) ? Convert.ToString(Request.QueryString["StoreID"]) : "") + "';", true);
                    }
                }
            }
        }
            ...
            else
            {
                ...
                if (Convert.ToInt32(ProductComponent.UpdateProduct(tb_Product)) > 0)
                {
                    ...
                    try
                    {
                        ...
                        {
                            strImageName = RemoveSpecialCharacter(txtSKU.Text.ToString().ToCharArray()) + "_" + ProductID + ".jpg";
                            string Imgname = Convert.ToString(tb_Product.ImageName);
        ...
        public tb_Product SetValue(tb_Product tb_Product)
        {
            ...
            // Added By Girish - start
            tb_Product.OptionSku = txtOptionSKU.Text.ToString().Trim();
            tb_Product.ShortName = txtShortName.Text.ToString().Trim();
            tb_Product.PartNumber = txtPartNumber.Text.ToString().Trim();
            tb_Product.WarrantyProvider = ddlProvider.SelectedItem.Text.ToString().Trim();
            tb_Product.WarrantyDescription = txtWarrantyDescription.Text.ToString().Trim();
            tb_Product.WarrantyContactPhoneNumber = txtWarrantyContactNumber.Text.ToString().Trim();
            tb_Product.Materials = txtMaterials.Text.ToString().Trim();
            tb_Product.ProductSummary = txtSummary.Text.ToString().Trim();
            tb_Product.SourceZipCode = txtSourceZipCode.Text.ToString().Trim();
            tb_Product.Brand = txtBrand.Text.ToString().Trim();
            if (ddlProductCondition.SelectedIndex != 0)
            {
                tb_Product.ProductCondition = ddlProductCondition.SelectedItem.Text.ToString().Trim();
            }
            // Added By Girish - end
        ...
        protected void btnCloneNewProduct_Click(object sender, EventArgs e)
        {
            try
            {
                if (Request.QueryString["oldid"] != null && !string.IsNullOrEmpty(Request.QueryString["oldid"].ToString()) && Request.QueryString["storeid"] != null && !string.IsNullOrEmpty(Request.QueryString["storeid"].ToString()))
                {
                    ...
                    url = "ProductOverStock.aspx?StoreID=" + CurrentStoreID.ToString() + "&ID=" + Request.QueryString["oldid"].ToString() + "&CloneID=" + Request.QueryString["CloneID"].ToString();
