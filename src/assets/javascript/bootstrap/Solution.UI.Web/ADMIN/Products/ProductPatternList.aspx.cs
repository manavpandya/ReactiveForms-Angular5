...
{
    public partial class ProductPatternList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    Page.ClientScript.RegisterStartupScript(Page.GetType(), "msg", "$(document).ready( function() {jAlert('Product Pattern inserted successfully.', 'Message');});", true);

                }
                else if (Request.QueryString["Update"] != null)
                {
                    Page.ClientScript.RegisterStartupScript(Page.GetType(), "msg", "$(document).ready( function() {jAlert('Product Pattern updated successfully.', 'Message');});", true);
                }
        ...
        private void binddata()
        {
            ...
            dsproduct = objproduct.GetProductPatternList(0, Issearch, SearchBy, SearchValue);
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow r in gvProductSize.Rows)
            {
                ...
                Label lb = (Label)r.FindControl("lblPatternID");
                int ID = Convert.ToInt32(lb.Text.ToString());
                if (chk.Checked)
                {
                    count++;
                    indx = ProductComponent.DeleteProductPattern(Convert.ToInt32(ID));
                }
            }
        ...
        protected void gvProductSize_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "edit")
            {
                try
                {
                    int PatternID = Convert.ToInt32(e.CommandArgument);
                    Response.Redirect("AddProductPattern.aspx?Mode=Edit&ID=" + PatternID);
                }
                catch
                { }
            }
        ...
        public String GetIconImageProduct(String img)
        {
            ...
            imagepath = AppLogic.AppConfigs("ImagePathProduct") + "Pattern/Micro/" + img;

            if (File.Exists(Server.MapPath(imagepath)))
            {
                return imagepath;
            }
            return string.Concat(AppLogic.AppConfigs("ImagePathProduct") + "Pattern/Micro/image_not_available.jpg");
        }
    }
}