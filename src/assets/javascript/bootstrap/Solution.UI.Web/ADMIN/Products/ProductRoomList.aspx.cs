...
{
    public partial class ProductRoomList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    Page.ClientScript.RegisterStartupScript(Page.GetType(), "msg", "$(document).ready( function() {jAlert('Product Room inserted successfully.', 'Message');});", true);

                }
                else if (Request.QueryString["Update"] != null)
                {
                    Page.ClientScript.RegisterStartupScript(Page.GetType(), "msg", "$(document).ready( function() {jAlert('Product Room updated successfully.', 'Message');});", true);
                }
        ...
        private void binddata()
        {
            ...
            dsproduct = objproduct.GetProductRoomList(0, Issearch, SearchBy, SearchValue);
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow r in gvProductSize.Rows)
            {
                ...
                Label lb = (Label)r.FindControl("lblRoomID");
                int ID = Convert.ToInt32(lb.Text.ToString());
                if (chk.Checked)
                {
                    count++;
                    indx = ProductComponent.DeleteProductRoom(Convert.ToInt32(ID));
                }
            }
        ...
        protected void gvProductSize_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "edit")
            {
                try
                {
                    ...
                    Response.Redirect("AddProductRoom.aspx?Mode=Edit&ID=" + RoomID);
                }
                catch
                { }
            }
        ...
        public String GetIconImageProduct(String img)
        {
            ...
            imagepath = AppLogic.AppConfigs("ImagePathProduct") + "Room/Micro/" + img;

            if (File.Exists(Server.MapPath(imagepath)))
            {
                return imagepath;
            }
            return string.Concat(AppLogic.AppConfigs("ImagePathProduct") + "Room/Micro/image_not_available.jpg");
        }
    }
}