...
{
    public partial class ProductStyleList : BasePage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    Page.ClientScript.RegisterStartupScript(Page.GetType(), "msg", "$(document).ready( function() {jAlert('Product Style inserted successfully.', 'Message');});", true);

                }
                else if (Request.QueryString["Update"] != null)
                {
                    Page.ClientScript.RegisterStartupScript(Page.GetType(), "msg", "$(document).ready( function() {jAlert('Product Style updated successfully.', 'Message');});", true);
                }
        ...
        private void binddata()
        {
            ...
            dsproduct = objproduct.GetProductStyleList(0, Issearch, SearchBy, SearchValue);
        ...
        protected void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow r in gvProductSize.Rows)
            {
                ...
                Label lb = (Label)r.FindControl("lblStyleID");
                int ID = Convert.ToInt32(lb.Text.ToString());
                if (chk.Checked)
                {
                    count++;
                    indx = ProductComponent.DeleteProductStyle(Convert.ToInt32(ID));
                }
            }
        ...
        protected void gvProductSize_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "edit")
            {
                try
                {
                    int StyleID = Convert.ToInt32(e.CommandArgument);
                    Response.Redirect("AddProductStyle.aspx?Mode=Edit&ID=" + StyleID);
                }
                catch
                { }
            }
        ...
        public String GetIconImageProduct(String img)
        {
            ...
            imagepath = AppLogic.AppConfigs("ImagePathProduct") + "Style/Micro/" + img;

            if (File.Exists(Server.MapPath(imagepath)))
            {
                return imagepath;
            }
            return string.Concat(AppLogic.AppConfigs("ImagePathProduct") + "Style/Micro/image_not_available.jpg");
        }
    }
}