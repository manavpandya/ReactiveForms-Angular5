...
{
    public partial class SetOnsaleDisplayOrder : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                DsOption = CommonComponent.GetCommonDataSet("select isnull(OrderbyCustom,0) as OrderbyCustom,isnull(OrderbyName,0) as OrderbyName,isnull(OrderbyPrice,0) as OrderbyPrice from tb_custompagemaster where Pagename='onsale'");
        ...
        private void BindProducts()
        {
            ...
            dsProducts = CommonComponent.GetCommonDataSet("exec GuiGetOnSaleProduct 1,0,'Price Range'");
        ...
        protected void btnsave_Click(object sender, EventArgs e)
        {
            ...
            {
                CommonComponent.ExecuteCommonData("update tb_custompagemaster set Orderbycustom=0,Orderbyname=1,orderbyprice=0 where pagename='onsale'");
                hdnoptions.Value = "name";
            }
            else if (rdooption.SelectedValue.ToString().ToLower() == "price")
            {
                CommonComponent.ExecuteCommonData("update tb_custompagemaster set Orderbycustom=0,Orderbyname=0,orderbyprice=1 where pagename='onsale'");
                hdnoptions.Value = "price";
            }
            else
            {
                CommonComponent.ExecuteCommonData("update tb_custompagemaster set Orderbycustom=1,Orderbyname=0,orderbyprice=0 where pagename='onsale'");
                hdnoptions.Value = "custom";
            }
