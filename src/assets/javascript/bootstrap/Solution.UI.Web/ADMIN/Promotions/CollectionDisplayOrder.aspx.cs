...
{
    public partial class CollectionDisplayOrder : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    ltrtitle.Text = Convert.ToString(CommonComponent.GetScalarCommonData("SELECT  Name from tb_Collection WHERE  CollectionId=" + Request.QueryString["id"].ToString() + "")) + " Product List.";
                }
                DataSet DsOption = new DataSet();
                DsOption = CommonComponent.GetCommonDataSet("select isnull(OrderbyCustom,0) as OrderbyCustom,isnull(OrderbyName,0) as OrderbyName,isnull(OrderbyPrice,0) as OrderbyPrice from tb_custompagemaster where Pagename='collection'");
        ...
        private void BindProducts()
        {
            ...
            {
                dsProducts = CommonComponent.GetCommonDataSet("EXEC usp_Product_CollectionAdmin '" + Request.QueryString["url"].ToString() + "'");
            }
            else
            {
                dsProducts = CommonComponent.GetCommonDataSet("EXEC usp_Product_CollectionAdmin");
            }
        ...
        protected void btnsave_Click(object sender, EventArgs e)
        {
            if (gridproducts.Rows.Count > 0)
            {
                ...
                CommonComponent.ExecuteCommonData("delete from tb_CollectionDisplayorder WHERE newEventId=" + Request.QueryString["id"].ToString() + "");
                ...
                for (int i = 0; i < gridproducts.Rows.Count; i++)
                {
                    ...
                    CommonComponent.ExecuteCommonData("insert into tb_CollectionDisplayorder(ProductID,DisplayOrder,newEventId) values (" + hdnProductid.Value.ToString() + "," + Display + "," + Request.QueryString["id"].ToString() + ") ");
                }
                if (rdooption.SelectedValue.ToString().ToLower() == "name")
                {
                    CommonComponent.ExecuteCommonData("update tb_custompagemaster set Orderbycustom=0,Orderbyname=1,orderbyprice=0 where pagename='collection'");
                    hdnoptions.Value = "name";
                }
                else if (rdooption.SelectedValue.ToString().ToLower() == "price")
                {
                    CommonComponent.ExecuteCommonData("update tb_custompagemaster set Orderbycustom=0,Orderbyname=0,orderbyprice=1 where pagename='collection'");
                    hdnoptions.Value = "price";
                }
                else
                {
                    CommonComponent.ExecuteCommonData("update tb_custompagemaster set Orderbycustom=1,Orderbyname=0,orderbyprice=0 where pagename='collection'");
                    hdnoptions.Value = "custom";
                }
