...
{
    public partial class CollectionList : BasePage
    {
        #region Declaration
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                if (!string.IsNullOrEmpty(Request.QueryString["status"]))
                {
                    ...
                    {
                        Page.ClientScript.RegisterStartupScript(Page.GetType(), "msg", "$(document).ready( function() {jAlert('Collection Details inserted successfully.', 'Message');});", true);
                    }
                    else if (strStatus == "updated")
                    {
                        Page.ClientScript.RegisterStartupScript(Page.GetType(), "msg", "$(document).ready( function() {jAlert('Collection Details updated successfully.', 'Message');});", true);
                    }
                }
                int loginid = 0;
        ...
        private void BindEventList()
        {
            ...
            {
                strwhere = " WHERE isnull(isactive,0)=1 ";
            }
            else if (ddlSearch.SelectedIndex == 2)
            {
                strwhere = " WHERE isnull(isactive,0)=0 ";
            }
            
            if (String.IsNullOrEmpty(txtSearch.Text))
            {
                dsevents = CommonComponent.GetCommonDataSet("select * from tb_Collection " + strwhere + " order by createdon desc");
            }
            else
            {
                strwhere = strwhere.Replace("WHERE", "AND");
                dsevents = CommonComponent.GetCommonDataSet("select * from tb_Collection where name like '%" + txtSearch.Text.Replace("'", "''") + "%' " + strwhere + "");
            }
        ...
        protected void gridcoupon_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            if (e.CommandName == "edit")
            {
                ...
                Response.Redirect("ExpireCollectionBanner.aspx?CollectionID=" + couponid + "&st=" + ddlSearch.SelectedValue.ToString() + "&txtse=" + Server.UrlEncode(txtSearch.Text.ToString()) + "");
            }
        }
        ...
        {
            Response.Redirect("CollectionList.aspx?st=" + ddlSearch.SelectedValue.ToString() + "&txtse=" + Server.UrlEncode(txtSearch.Text.ToString()) + "");
            //DataSet dsevents = new DataSet();
        ...
        protected void gridcoupon_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                alinkdisplay.HRef = "CollectionDisplayOrder.aspx?id=" + hdneventid.Value.ToString() + "&url=" + hdnurlname.Value.ToString();
            }
            ...
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                HiddenField eventactive = (HiddenField)e.Row.FindControl("hdeventactive");
                ...
                //else
                {
                    ...
                    //    e.Row.Visible = false;
                    //}
                    if (eventactive.Value.ToString() == "1" || eventactive.Value.ToString().ToLower() == "true")
                    {
                        ltStatusColor.Text = "<div style='float:left; width:10px; height:10px; background-color:green'></div>&nbsp;";
                    }
                    else
        ...
        protected void btndelete_Click(object sender, EventArgs e)
        {
            ...
            for (int tt = 0; tt < totalRowCount; tt++)
            {
                ...
                if (chk.Checked == true)
                {
                    ...
                    string couponid = Convert.ToString(CommonComponent.GetScalarCommonData("select isnull(couponcode,'') as CouponCode from tb_Collection where CollectionId=" + hdn.Value.ToString() + ""));
                    ...
                    CommonComponent.ExecuteCommonData("delete from tb_Collection where CollectionId=" + hdn.Value.ToString() + "");
                    if (!String.IsNullOrEmpty(couponid))
                    {
                        DataSet dspids = new DataSet();
                        dspids = CommonComponent.GetCommonDataSet("select isnull(Productids,'') as Productids from tb_Collection where couponcode='" + couponid.ToString().Trim() + "' and isnull(Productids,'')<>''");
