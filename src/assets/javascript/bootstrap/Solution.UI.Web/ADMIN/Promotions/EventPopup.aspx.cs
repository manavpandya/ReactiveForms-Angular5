...
{
    public partial class EventPopup : Solution.UI.Web.BasePage
    {
        #region Declaration
        ...
        private void BindSelectedProduct()
        {
            ...
            string ids = "";
            if (Request.QueryString["eventid"] != null && Request.QueryString["eventid"] != "")
            {
                if (Session["selectedpid"] != null && Session["selectedpid"].ToString() != "")
                {
                    try
                ...
                if (!string.IsNullOrEmpty(ids) && ids.Length > 2)
                {
                    ...
                    dsselectedproduct = CommonComponent.GetCommonDataSet("select productid,isnull(sku,'') as sku,isnull(name,'') as name from tb_product where productid in (SELECT DISTINCT items FROM dbo.split('" + ids + "',',') WHERE isnull(Items,'')<>'') and isnull(deleted,0)=0");
        ...
        private void getselectedproducts()
        {
            ...
            if (Request.QueryString["eventid"] != null && Request.QueryString["eventid"] != "")
            {
                int couponid = Convert.ToInt32(Request.QueryString["eventid"].ToString());

                if (Session["pidevent"] != null && Session["pidevent"].ToString() != "")
                {
                    ids = Session["pidevent"].ToString();
                }
                else
                {
                    // tb_coupon = objcoupon.Getcoupon(couponid);

                    ids = Convert.ToString(CommonComponent.GetScalarCommonData("select isnull(Productids,'') as Productids from tb_Event where eventid=" + couponid + ""));
