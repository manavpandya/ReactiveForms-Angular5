...
{
    public partial class GiftCardProduct : BasePage
    {
        int storeID = 1;
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["StoreID"] != null && !string.IsNullOrEmpty(Request.QueryString["StoreID"]))
        ...
        public void BindData(Int32 ID, Int32 StoreID)
        {
            ...
            if (DsProduct != null && DsProduct.Tables.Count > 0 && DsProduct.Tables[0].Rows.Count > 0)
            {
                ...
                txtSEPageTitle.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["SETitle"]);
                txtSEKeywords.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["SEKeywords"]);
                txtSEDescription.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["SEDescription"]);
                txtavailablestartdate.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["AvaliableStartDate"]);
                txtavailableenddate.Text = Convert.ToString(DsProduct.Tables[0].Rows[0]["AvailableEndDate"]);
                txtourprice.Text = Convert.ToDecimal(DsProduct.Tables[0].Rows[0]["OurPrice"]).ToString("F2");
        ...
        public void InsertProduct()
        {
            ...
            //else
            tb_Product.tb_StoreReference.EntityKey = new EntityKey("RedTag_CCTV_Ecomm_DBEntities.tb_Store", "StoreID", Convert.ToInt32(ddlStore.SelectedValue));
            ...
            tb_Product.Price = Convert.ToDecimal(OurPrice);
            tb_Product.SalePrice = Convert.ToDecimal(OurPrice);
            tb_Product.Description = Convert.ToString((txtDescription.Text.Trim()));
            tb_Product.SETitle = txtSEPageTitle.Text.Trim();
            tb_Product.SEKeywords = txtSEKeywords.Text.ToString().Trim();
            tb_Product.SEDescription = txtSEDescription.Text.Trim();
            if (!string.IsNullOrEmpty(txtavailablestartdate.Text.Trim()))
                tb_Product.AvaliableStartDate = null;//Convert.ToDateTime(txtavailablestartdate.Text);
            if (!string.IsNullOrEmpty(txtavailableenddate.Text.Trim()))
                tb_Product.AvailableEndDate = null;// Convert.ToDateTime(txtavailableenddate.Text);
            tb_Product.CreatedBy = Convert.ToInt32(Session["AdminID"]);
            tb_Product.CreatedOn = System.DateTime.Now;
            tb_Product.Deleted = false;
            tb_Product.Active = Convert.ToBoolean(chkIsActive.Checked);

            Int32 ChkSKu = Convert.ToInt32(CommonComponent.GetScalarCommonData("SElect ISNULL(count(sku),0) as TotCnt From tb_product Where sku='" + txtSKU.Text.ToString().Trim() + "' And StoreId=" + ddlStore.SelectedValue.ToString() + " AND ISNULL(Deleted,0)=0 "));
            if (ChkSKu > 0)
            {
            ...
            else
            {
                ...
                if (ProductID > 0)
                {
                    ProductComponent.InsertGiftcardproduct(Convert.ToInt32(ddlStore.SelectedValue.ToString()), ProductID);
                    string strImageName = "";
                    ...
                    Response.Redirect("GiftCardProductList.aspx?Insert=true&StoreID=" + ddlStore.SelectedValue.ToString());
                }
            }
        }
        ...
        public void UpdateProduct(int ProductID)
        {
            ...
            tb_Product.SEDescription = txtSEDescription.Text.Trim();
            if (!string.IsNullOrEmpty(txtavailablestartdate.Text.Trim()))
                tb_Product.AvaliableStartDate = Convert.ToDateTime(txtavailablestartdate.Text);
            if (!string.IsNullOrEmpty(txtavailableenddate.Text.Trim()))
                tb_Product.AvailableEndDate = Convert.ToDateTime(txtavailableenddate.Text);
            ...
            else
            {
                ...
                if (Convert.ToInt32(ProductComponent.UpdateProduct(tb_Product)) > 0)
                {
                    ...
                    ProductComponent.UpdateProduct(tb_Product);
                    Response.Redirect("GiftCardProductList.aspx?Update=true&StoreID=" + ((Request.QueryString["StoreID"] != null) ? Convert.ToString(Request.QueryString["StoreID"]) : ""));
                }
            }
        }
        ...
        protected void btnDelete_Click(object sender, ImageClickEventArgs e)
        {
            ...
            else
            {
                ...
                {
                    ImgLarge.Src = "";
                    String strProductMediumPath = string.Concat(AppLogic.AppConfigs("ImagePathProduct"), "Medium/");
        ...
        {
            Response.Redirect("GiftCardProductList.aspx");
        }
