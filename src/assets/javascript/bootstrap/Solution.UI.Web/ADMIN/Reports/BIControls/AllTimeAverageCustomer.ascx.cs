...
{
    public partial class AllTimeAverageCustomer : System.Web.UI.UserControl
    {
        ...
        public void CreateChart(string DBAction)
        {
            ...
            int TotalOrders = 0;
            ds = adminComponent.GetAllTimeAverageCustomer(DBAction, StartDate, EndDate);
            string str = string.Empty;
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                str = str + "[['Duration','AverageCustomer'],";
                int cnt = ds.Tables[0].Rows.Count;
                int i = 1;
                if (cnt > 0)
                {
                    TotalOrders = Convert.ToInt32(Convert.ToString(ds.Tables[0].Rows[cnt - 1]["AverageCustomer"]));
                }
                foreach (DataRow item in ds.Tables[0].Rows)
                {
                    str = str + "['" + item["Duration"].ToString() + "'," + Convert.ToString(item["AverageCustomer"]) + "]";
                    if (i != cnt)
                    {
                        str = str + ",";
                    }
            ...
            lblOrderAverage.ToolTip = "Total Customer";

            strScript += "google.load('visualization', '1', { packages: ['corechart'] });";
            strScript += "google.setOnLoadCallback(AverageCustomerdiv);";
            strScript += "Sys.Application.add_load(AverageCustomerdiv);";
            strScript += "function AverageCustomerdiv() {";
            ...
            strScript += "var options = {title: 'All Time Average Customer',pointSize: 5,vAxis: {title: 'Average Customer', minValue: 0, titleTextStyle: {color: '#000000'}},hAxis:{title: '" + hAxis + "', titleTextStyle: {color: '#000000'}}};";
            strScript += "var chart = new google.visualization.AreaChart(document.getElementById('AverageCustomerdiv'));";
