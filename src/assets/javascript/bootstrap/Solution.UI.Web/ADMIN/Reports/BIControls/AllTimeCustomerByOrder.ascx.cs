...
{
    public partial class AllTimeCustomerByOrder : System.Web.UI.UserControl
    {
        ...
        public void CreateChart(string DBAction)
        {
            ...
            ds = adminComponent.GetCustomersByOrder(DBAction, StartDate, EndDate);
            ...
            if (ds != null && ds.Tables[0].Rows.Count > 0)
            {
                foreach (DataRow item in ds.Tables[0].Rows)
                {
                    ...
                    str = str + "['" + item["Customer"].ToString() + "', " + Convert.ToString(item["TotalOrder"]) + ",  {v: " + Convert.ToString(item["TotalRevenue"]) + ", f: '$" + Convert.ToString(item["TotalRevenue"]) + "'}],";
                }
            }
            else
            {
            ...
            strScript += "google.setOnLoadCallback(CustomerOrderdrawChart);";
            strScript += "Sys.Application.add_load(CustomerOrderdrawChart);";
            strScript += "function CustomerOrderdrawChart() {";
            ...
            strScript += str;
            strScript += "var table = new google.visualization.Table(document.getElementById('CustomerByOrderschartdiv'));";
            strScript += " table.draw(data, {showRowNumber: true});";
            strScript += "}";
            ScriptManager.RegisterStartupScript(upnlCustomerOrder, upnlCustomerOrder.GetType(), "All Time Customer by Order", strScript, true);
        }
