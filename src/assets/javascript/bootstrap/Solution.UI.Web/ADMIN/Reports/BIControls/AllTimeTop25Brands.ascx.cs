...
{
    public partial class AllTimeTop25Brands : System.Web.UI.UserControl
    {
        ...
        public void CreateChart(string DBAction)
        {
            try
            {
                ...
                ds = adminComponent.GetAllTimeTop25Brands(DBAction, StartDate, EndDate);
                string str = string.Empty;
                int TotalCustomer = 0;
                if (ds != null && ds.Tables[0].Rows.Count > 0)
                {
                    str = str + "[['Brand Name','Quantity'],";
                    int cnt = ds.Tables[0].Rows.Count;
                    int i = 1;
                    foreach (DataRow item in ds.Tables[0].Rows)
                    {
                        TotalCustomer = TotalCustomer + Convert.ToInt32(Convert.ToString(item["Quantity"]));

                        str = str + "['" + item["Brand"].ToString() + "'," + item["Quantity"].ToString() + "]";
                        if (i != cnt)
                        {
                            str = str + ",";
                        }
                ...
                strScript += "google.setOnLoadCallback(Top25Brandschartdiv);";
                strScript += "Sys.Application.add_load(Top25Brandschartdiv);";
                strScript += "function Top25Brandschartdiv() {";
                strScript += "var data = google.visualization.arrayToDataTable(" + str + ");";
                strScript += "var options = {title: 'Top 10 Brand',pieHole: 0.4,is3D:true};";
                strScript += "var chart = new google.visualization.PieChart(document.getElementById('Top25Brandschartdiv'));";
                strScript += "chart.draw(data, options);";
                strScript += "}";
                ScriptManager.RegisterStartupScript(this, this.GetType(), "All Time Top 25 Brands", strScript, true);
            }
            catch (Exception)
            {
            }
