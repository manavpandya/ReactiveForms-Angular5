...
namespace Solution.UI.Web.ADMIN.Reports
{
    public partial class ContactInquiries : BasePage
    {
        ...
        private void GetContact()
        {
            ...
            {
                UserType = Convert.ToInt32(Session["AdminType"].ToString());
            }
            if (UserType == 0)
            {
                 strSql = "SELECT * FROM tb_ContactUs WHERE ContactUsID <> 0 ";
            }
            else
            {
               
                strSql = "SELECT * FROM tb_ContactUs WHERE ContactUsID <> 0  and tb_ContactUs.Email='"+Session["AdminEmailID"].ToString()+"'";
        ...
        protected void grvContactReport_RowDataBound(object sender, GridViewRowEventArgs e)
        {

            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                System.Web.UI.HtmlControls.HtmlAnchor ltassign = (System.Web.UI.HtmlControls.HtmlAnchor)e.Row.FindControl("ltassign");
                int UserType = 1;
                ...
                if(UserType==0)
                {
                    ltassign.Visible = true;
                }
                else
                {
                    ltassign.Visible = false;
                }
            ...
            //{
            //    HyperLink ltassign = (HyperLink)e.Row.FindControl("ltassign");
            //    int UserType = 1;
