...
{
    public partial class EffHomePagebannerlist : BasePage
    {
        ...
        private void GetGroupGrid()
        {
            ...
            dsBannergroups = CommonComponent.GetCommonDataSet("select HomeRotatorId,StoreID,BannerTypeId,isnull(IsActive,0) as IsActive,isnull(BannerID,0) as BannerID,isnull(DisplayOrder,0) as DisplayOrder from tb_RotatorHomebannerEFF order by isnull(DisplayOrder,0) ASC");
            if (dsBannergroups != null && dsBannergroups.Tables.Count > 0 && dsBannergroups.Tables[0].Rows.Count > 0)
            {
                for (int kk = 0; kk < dsBannergroups.Tables[0].Rows.Count; kk++)
                {
                  //  dsbanner = objbanner.GetHomeRotatorBanner(Convert.ToInt32(ddlStore.SelectedValue.ToString()), Convert.ToInt32(dsBannergroups.Tables[0].Rows[kk]["HomeRotatorId"].ToString()));
                    dsbanner = CommonComponent.GetCommonDataSet("EXEC usp_HomeRotatorBannerEFF " + Convert.ToInt32(dsBannergroups.Tables[0].Rows[kk]["HomeRotatorId"].ToString()) + "," + Convert.ToInt32(ddlStore.SelectedValue.ToString()) + "");
                    ltactive.Text = "";
                    if (dsbanner != null && dsbanner.Tables.Count > 0 && dsbanner.Tables[0].Rows.Count > 0)
                    {
                        string path = Convert.ToString(CommonComponent.GetScalarCommonData("SELECT TOP 1 ConfigValue FROM tb_AppConfig WHERE ConfigName='EFFImagePathBanner' AND storeid=" + ddlStore.SelectedValue.ToString() + " AND isnull(Deleted,0)=0"));
            ...
            foreach (string strky in strkeys)
            {
                if (strky.ToString().ToLower().IndexOf("text1_" + hdnlayouid.Value.ToString()) > -1)
                {
                    if (Request.Form[strky] != null)
                    {
                        ...
                        {
                            CommonComponent.ExecuteCommonData("UPDATE tb_RotatorHomebannerEFF SET DisplayOrder=" + Request.Form[strky].ToString() + " WHERE HomeRotatorId=" + hdnlayouid.Value.ToString() + "");
                        }
                        else
                        {
                            CommonComponent.ExecuteCommonData("UPDATE tb_RotatorHomebannerEFF SET DisplayOrder=0 WHERE HomeRotatorId=" + hdnlayouid.Value.ToString() + "");
                        }
                    }
                }
            }
        ...
        protected void btnActive_Click(object sender, EventArgs e)
        {
            ...
            CommonComponent.ExecuteCommonData("UPDATE tb_RotatorHomebannerEFF SET IsActive=1 WHERE StoreId=" + Convert.ToInt32(ddlStore.SelectedValue.ToString()) + " AND HomeRotatorId=" + Convert.ToInt32(hdnTypeid.Value.ToString()) + "");
            string strHTML = "";
            HomeBannerComponent objbanner = new HomeBannerComponent();
            DataSet dsbanner = new DataSet();
            ViewState["ImagebannrpathEFF"] = CommonComponent.GetScalarCommonData("SELECT TOP 1 ConfigValue FROM tb_AppConfig WHERE ConfigName='EFFImagePathBanner' AND storeid=" + ddlStore.SelectedValue.ToString() + " AND isnull(Deleted,0)=0");
            ...
            dsbanner = CommonComponent.GetCommonDataSet("EXEC usp_HomeRotatorBannerEFF " + Convert.ToInt32(hdnTypeid.Value.ToString()) + "," + Convert.ToInt32(ddlStore.SelectedValue.ToString()) + "");
            string strPosition = "";
                ...
                if (dsbanner != null && dsbanner.Tables.Count > 0 && dsbanner.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr1 in dr)
                    {
                        ...
                        if (File.Exists(Server.MapPath(ViewState["ImagebannrpathEFF"].ToString()).ToString() + "/" + dr1["ImageName"].ToString()))
                        {
                            ...
                            {
                                strHTML += " <div class='banner2'><a href=\"" + dr1["BannerURL"].ToString() + "\" target=\"" + dr1["LinkTarget"].ToString().Trim() + "\"><img style='cursor:pointer;' height='227' width='980' src='" + ViewState["ImagebannrpathEFF"].ToString() + "/" + dr1["ImageName"].ToString() + "' alt='" + dr1["Title"].ToString() + "' Title='" + dr1["Title"].ToString() + "' /></a></div>";
                            }
                            else
                            {
                                strHTML += " <div class='banner2'><img  height='227' width='980' src='" + ViewState["ImagebannrpathEFF"].ToString() + "/" + dr1["ImageName"].ToString() + "' alt='" + dr1["Title"].ToString() + "' Title='" + dr1["Title"].ToString() + "' /></div>";
                            }
                            strHTML += " </div>";
                            strHTML += " </div>";
                        }
            ...
            else if (Request.QueryString["id"] != null && Request.QueryString["id"].ToString() == "2")
            {
                ...
                foreach (DataRow dr1 in drSmall)
                {
                    ...
                    if (File.Exists(Server.MapPath(ViewState["ImagebannrpathEFF"].ToString()).ToString() + "/" + dr1["ImageName"].ToString()))
                    {
                        ...
                        if (dr1["BannerURL"].ToString().Trim() != "")
                        {
                            strHTML += "<div class=\"sub-banner-" + Count + "\"> <a title=\"" + dr1["Title"].ToString() + "\" href=\"/engravingfonts\"><img title=\"" + dr1["Title"].ToString() + "\" alt=\"" + dr1["Title"].ToString() + "\" src=\"" + ViewState["ImagebannrpathEFF"].ToString() + "/" + dr1["ImageName"].ToString() + "\"></a></div>";
                        }
                        else
                        {
                            strHTML += "<div class=\"sub-banner-" + Count + "\"><img title=\"" + dr1["Title"].ToString() + "\" alt=\"" + dr1["Title"].ToString() + "\" src=\"" + ViewState["ImagebannrpathEFF"].ToString() + "/" + dr1["ImageName"].ToString() + "\"></div>";
                        }
                    }
                }
            }
                ...
                if (dsbanner != null && dsbanner.Tables.Count > 0 && dsbanner.Tables[0].Rows.Count > 0)
                {
                    ...
                    foreach (DataRow dr1 in dr)
                    {
                        ...
                        if (File.Exists(Server.MapPath(ViewState["ImagebannrpathEFF"].ToString()).ToString() + "/" + dr1["ImageName"].ToString()))
                        {
                            ...
                            if (dr1["BannerURL"].ToString().Trim() != "")
                            {
                                strHTML += " <div class='banner2'><a href=\"" + dr1["BannerURL"].ToString() + "\" target=\"" + dr1["LinkTarget"].ToString().Trim() + "\"><img style='cursor:pointer;' height='227' width='590' src='" + ViewState["ImagebannrpathEFF"].ToString() + "/" + dr1["ImageName"].ToString() + "' alt='" + dr1["Title"].ToString() + "' Title='" + dr1["Title"].ToString() + "' /></a></div>";
                            }
                            else
                            {
                                strHTML += " <div class='banner2'><img  height='227' width='590' src='" + ViewState["ImagebannrpathEFF"].ToString() + "/" + dr1["ImageName"].ToString() + "' alt='" + dr1["Title"].ToString() + "' Title='" + dr1["Title"].ToString() + "' /></div>";
                            }
                            strHTML += " </div>";
                            strHTML += " </div>";
                        }
            ...
            Int32 StoreId = Convert.ToInt32(CommonComponent.GetScalarCommonData("SELECT TOp 1 StoreId FROM tb_HtmlTextEFF WHERE StoreId=" + ddlStore.SelectedValue.ToString() + " AND HtmlType=1"));
            if (StoreId > 0)
            {
                CommonComponent.ExecuteCommonData("UPDATE tb_HtmlTextEFF SET HtmlText='" + strHTML.Replace("'", "''") + "' WHERE StoreId=" + ddlStore.SelectedValue.ToString() + " AND HtmlType=1");
            }
            else
            {
                CommonComponent.ExecuteCommonData("INSERT INTO tb_HtmlTextEFF (StoreId,HtmlText,HtmlType) values (" + ddlStore.SelectedValue.ToString() + ",'" + strHTML.Replace("'", "''") + "',1)");
            }
        ...
        {
            CommonComponent.ExecuteCommonData("UPDATE tb_RotatorHomebannerEFF SET IsActive=0 WHERE StoreId=" + Convert.ToInt32(ddlStore.SelectedValue.ToString()) + " AND HomeRotatorId=" + Convert.ToInt32(hdnTypeid.Value.ToString()) + "");
            GetGroupGrid();
            BindBanner();
            ...
            foreach (string strky in strkeys)
            {
                if (strky.ToString().ToLower().IndexOf("text1_" + hdnlayouid.Value.ToString()) > -1)
                {
                    if (Request.Form[strky] != null)
                    {
                        if (Request.Form[strky].ToString() != "")
                        {
                            CommonComponent.ExecuteCommonData("Delete from tb_RotatorHomeBannerDetailEFF WHERE HomeRotatorId=" + hdnlayouid.Value.ToString() + "");
                            CommonComponent.ExecuteCommonData("Delete from tb_RotatorHomebannerEFF WHERE HomeRotatorId=" + hdnlayouid.Value.ToString() + "");
                        }
