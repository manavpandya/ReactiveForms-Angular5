...
{
    public partial class HomepageRightbanner : BasePage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                {
                    ViewState["Imagebannrpath"] = CommonComponent.GetScalarCommonData("SELECT TOP 1 ConfigValue FROM tb_AppConfig WHERE ConfigName='ImagePathRightBanner' AND storeid=" + Request.QueryString["Storeid"].ToString() + " AND isnull(Deleted,0)=0");
                    Getbanner();
                }
                //if (Request.QueryString["id"] != null && Request.QueryString["id"].ToString() == "1")
                //{

                lblImgSize.Text = "Size should be 454 x 648 ";
                //}
        ...
        private void Getbanner()
        {
            ...
            dsbanner = CommonComponent.GetCommonDataSet("SELECT * FROM tb_HomeRightBannerDetail WHERE StoreId=" + Request.QueryString["StoreId"].ToString() + ""); //objbanner.GetHomeRotatorBanner(Convert.ToInt32(Request.QueryString["StoreId"].ToString()), Convert.ToInt32(Request.QueryString["id"].ToString()));
        ...
        protected void imgSave_Click(object sender, ImageClickEventArgs e)
        {
            ...
            {
                string strSql = "";
                strSql = "INSERT INTO tb_HomeRightBannerDetail(StoreID,Title,BannerURL,ImageName,DisplayOrder,Active,CreatedOn,Description,Pagination,IsMain,LinkTarget,StartDate,EndDate)";
                ...
                if (FileUploadBanner.HasFile)
                {
                    ...
                    CommonComponent.ExecuteCommonData("UPDATE tb_HomeRightBannerDetail SET ImageName='" + filename + "' WHERE BannerID=" + bannerId + "");
                }
            }
            else
            {
                ...
                CommonComponent.ExecuteCommonData("UPDATE tb_HomeRightBannerDetail SET Title='" + TxtbannerTitle.Text.ToString().Replace("'", "''") + "',BannerURL='" + TxtBannerURL.Text.ToString().Replace("'", "''") + "',DisplayOrder=" + TxtDisplayOrder.Text.ToString() + ",Active=" + Active.ToString() + ",Description='" + txtDescription.Text.ToString().Replace("'", "''") + "',Pagination='" + ddlPosition.SelectedValue.ToString() + "',LinkTarget='" + ddlTarget.SelectedValue.ToString() + "',StartDate='" + StartDate.ToString() + "',EndDate='" + EndDate.ToString() + "', ImageName='" + filename + "' WHERE BannerID=" + bannerId + "");
            }
            hdnid.Value = "0";
            imghdn.Value = "";
            TxtbannerTitle.Text = "";
        ...
        protected void grdbannerlist_RowDeleting(object sender, GridViewDeleteEventArgs e)
        {
            ...
            CommonComponent.ExecuteCommonData("Delete from tb_HomeRightBannerDetail where BannerID=" + id + "");
            if (File.Exists(Server.MapPath(ViewState["Imagebannrpath"].ToString() + "/" + filename)))
            {
                File.Delete(Server.MapPath(ViewState["Imagebannrpath"].ToString() + "/" + filename));
                CommonOperations.DeleteFileFromContentServer(Server.MapPath(ViewState["Imagebannrpath"].ToString() + "/" + filename));
