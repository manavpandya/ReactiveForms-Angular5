...
{
    public partial class CheckoutCommon : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                {
                    Session["CustID"] = Request.QueryString["CID"].ToString();
        ...
        private void AddOrderWithLogin()
        {
            ...
            try
            {
                if (strShippingName.ToLower().Trim() != "ground")
                {
                    Session["Emailforshippingmethod"] = Convert.ToString(AppLogic.AppConfigs("Emailforshippingmethod").ToString());
                }
                else
                {
            ...
            try
            {
                if (String.IsNullOrEmpty(txtBillEmail.Text))
                {
                    if (ViewState["BillEmail"] != null)
                    {
                        txtBillEmail.Text = ViewState["BillEmail"].ToString();
                    }
                }
                if (String.IsNullOrEmpty(txtShipEmailAddress.Text))
                {
                    if (ViewState["BillEmail"] != null)
                    {
                        txtShipEmailAddress.Text = ViewState["BillEmail"].ToString();
                    }
                }
            }
            catch { }
                                                ...
                                                for (int j = 0; j < strNmyard.Length; j++)
                                                {
                                                    if (strNmyard[j].ToString().Trim().ToLower().IndexOf("select size") > -1)
                                                    {
                                                        ...
                                                        else
                                                        {
                                                            ...
                                                            pInventory = CntInv;
                                                            string Ismadetoorder = Convert.ToString(CommonComponent.GetScalarCommonData("SELECT TOP 1 isnull(Ismadetoorder,0) FROM tb_product WHERE ProductId=" + hdnProductId.Value.ToString() + ""));
                                                            if (!string.IsNullOrEmpty(Ismadetoorder) && Convert.ToBoolean(Ismadetoorder)) // && CntInv < Convert.ToInt32(txtQty.Text.ToString()) 03/29/2014
                                                            {
                                                                string resp = "";
        ...
        private void UpdateMinicart()
        {
            ...
            if (Session["CustID"] != null && Session["CustID"].ToString() != "")
            {
                ...
                if (dsCart != null && dsCart.Tables.Count > 0 && dsCart.Tables[0].Rows.Count > 0)
                {
                    ...
                    decimal Price = Convert.ToDecimal(lblSubtotal.Text.ToString().Replace("$", ""));
        ...
        private void PaypalCheckout(tb_Order objOrder)
        {
            ...
            if (Request.QueryString["stype"] != null)
            {
                ...
                {
                    String sURL = gw.StartEC(strTranscationMode, "/checkoutcommon.aspx?resetlinkback=1");
                    Response.Redirect(sURL);
                }
            }
            else
        ...
        {
            Response.Redirect("/CheckoutCommon.aspx?stype=1");

        }

        protected void btnimgCreateNewAccount_Click(object sender, ImageClickEventArgs e)
        {
            Response.Redirect("/CheckoutCommon.aspx?stype=2");
        }
    }
}
