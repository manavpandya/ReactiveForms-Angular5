...
namespace Solution.UI.Web
{
    ...
    public partial class EditAddress : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                if (Request.QueryString["AddType"] != null && Request.QueryString["Type"] != null)
                {
                    ...
                    if (Request.QueryString["Type"].ToString() != "new")
                    {
                        ...
                        {
                            FillAddressData(Convert.ToInt32(Session["CustID"].ToString()));
                        }
                    }
                    else
                    {
        ...
        private void FillAddressData(Int32 CustomerID)
        {
            ...
            if (dsCustomerDetail != null && dsCustomerDetail.Tables.Count > 0 && dsCustomerDetail.Tables[0].Rows.Count > 0)
            {
                ...
                if (dsGetAddress != null && dsGetAddress.Tables.Count > 0 && dsGetAddress.Tables[0].Rows.Count > 0)
                {
                    ...
                    if (AddressID == Convert.ToInt32(dsCustomerDetail.Tables[0].Rows[0][AddType]))
                    {
            ...
            try
            {
                if (Request.QueryString["type"] != null && Convert.ToString(Request.QueryString["type"]) != "edit")
                {
                    if (Request.QueryString["AddType"] != null && Request.QueryString["AddType"].ToString() == "billingadd")
                    {
                        ...
                        {
                            Page.ClientScript.RegisterStartupScript(Page.GetType(), "InsertB", "alert('Billing Address is added successfully.');", true);
                        ...
                        {
                            Page.ClientScript.RegisterStartupScript(Page.GetType(), "FailInsert", "alert('Problem in adding billing address, please try again.');", true);
                            return;
                        }
                    }
                    if (Request.QueryString["AddType"] != null && Request.QueryString["AddType"].ToString() == "shippingadd")
                    {
                        ...
                        {
                            Page.ClientScript.RegisterStartupScript(Page.GetType(), "InsertS", "alert('Shipping Address is added successfully.');", true);
                            //Response.Redirect("AddressBook.aspx?msg=Shipping Address is added successfully...");
                            Response.Redirect("AddressBook.aspx");
                        }
                        else
                        {
                            Page.ClientScript.RegisterStartupScript(Page.GetType(), "FailInsert", "alert('Problem in adding shipping address, please try again.');", true);
                            return;
                        }
                    }
                }
                else if (Request.QueryString["type"] != null && Convert.ToString(Request.QueryString["type"]) == "edit")
                {
                    if (Request.QueryString["AddType"] != null && Request.QueryString["AddType"].ToString() == "billingadd")
                    {
                        ...
                        {
                            Page.ClientScript.RegisterStartupScript(Page.GetType(), "UpdateB", "alert('Billing Address is updated successfully.');", true);
                            // Response.Redirect("AddressBook.aspx?msg=Billing Address is updated successfully...");
                            Response.Redirect("AddressBook.aspx");
                        }
                        else
                        {
                            Page.ClientScript.RegisterStartupScript(Page.GetType(), "FailUpdating", "alert('Problem in updating billing address, please try again.');", true);
                            return;
                        }
                    }
                    if (Request.QueryString["AddType"] != null && Request.QueryString["AddType"].ToString() == "shippingadd")
                    {
                        ...
                        {
                            Page.ClientScript.RegisterStartupScript(Page.GetType(), "UpdateS", "alert('Shipping Address is updated successfully.');", true);
                            //Response.Redirect("AddressBook.aspx?msg=Shipping Address is updated successfully...");
                            Response.Redirect("AddressBook.aspx");
                        }
                        else
                        {
                            Page.ClientScript.RegisterStartupScript(Page.GetType(), "FailUpdating", "alert('Problem in updating shipping address, please try again.');", true);
                            return;
                        }
                    }
                }
        ...
        protected bool InsertBillingAddress()
        {
            ...
            {
                IsBillingAddressAdded = objCustomer.UpdateCustomerBillingAddress(Convert.ToInt32(Session["CustID"]), AddID);
            }
            ClearField();
        ...
        protected bool InsertShippingAddress()
        {
            ...
            if (chkDefaultAddress.Checked)
            {
                IsShippingAddressAdded = objCustomer.UpdateCustomerShippingAddress(Convert.ToInt32(Session["CustID"]), AddID);
            }
            ClearField();
        ...
        protected bool UpdateBillingAddress()
        {
            ...
            if (chkDefaultAddress.Checked)
            {
                IsBillingAddressUpdate = objCustomer.UpdateCustomerBillingAddress(Convert.ToInt32(Session["CustID"]), AddressID);
            }
            else
            {
                IsBillingAddressUpdate = objCustomer.UpdateCustomerBillingAddress(Convert.ToInt32(Session["CustID"]), 0);
            }
            ClearField();
        ...
        protected bool UpdateShippingAddress()
        {
            ...
            if (chkDefaultAddress.Checked)
            {
                IsShippingAddressUpdated = objCustomer.UpdateCustomerShippingAddress(Convert.ToInt32(Session["CustID"]), AddressID);
            }
            else
            {
                IsShippingAddressUpdated = objCustomer.UpdateCustomerShippingAddress(Convert.ToInt32(Session["CustID"]), 0);
            }
            ClearField();
        ...
        public tb_Address SetBillOrShipValue(Int32 AddressType)
        {
            ...
            tb_Address.CustomerID = Convert.ToInt32(Session["CustID"]);
