...
namespace Solution.UI.Web
{
    public partial class forgotpassword : System.Web.UI.Page
    {
        ...
        {
            if (Session["forgotemail"] != null && Session["forgotemail"].ToString().ToLower().Trim() == Email.ToLower().Trim())
            {
                Email = Session["forgotemail"].ToString();
            }
        ...
        private void SendMail()
        {
            ...
            if (dsMailTemplate != null && dsMailTemplate.Tables.Count > 0 && dsMailTemplate.Tables[0].Rows.Count > 0)
            {
                ...
                Int32.TryParse(Convert.ToString(CommonComponent.GetScalarCommonData("Select CustomerID from tb_Customer Where Email='" + lblEmail.Text + "' ans Storeid =" + AppLogic.AppConfigs("StoreID") + "and Isnull(IsRegistered,0) =1 ")), out CustomerID);

                CommonComponent.ExecuteCommonData("update tb_customer set password ='" + Convert.ToString(ViewState["Password"]) + "' where CustomerId =" + CustomerID);
