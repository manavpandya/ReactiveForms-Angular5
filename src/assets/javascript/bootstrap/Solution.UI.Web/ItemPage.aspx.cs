...
FROM dbo.tb_ProductOptionsPrice INNER JOIN dbo.tb_ProductSearchType ON dbo.tb_ProductOptionsPrice.Options = dbo.tb_ProductSearchType.SearchValue WHERE isnull(tb_ProductOptionsPrice.Active,0)=1 AND tb_ProductOptionsPrice.ProductId=" + Request.QueryString["PID"].ToString() + @" Order By tb_ProductSearchType.DisplayOrder");
        ...
        private void breadcrumbs()
        {
            ...
            if (Request.QueryString["PID"] != null)
            {
                ...
                " INNER join tb_CategoryMapping On tb_Category.CategoryID= tb_CategoryMapping.CategoryID WHERE  tb_Category.StoreID=" + Convert.ToInt32(1) + " And  tb_Product.StoreID=" + Convert.ToInt32(1) + " And tb_Category.Deleted=0 and  (tb_ProductCategory.ProductID = " + Request.QueryString["PID"] + ") ";
        ...
        private void GetProductDetails(int productID)
        {
            ...
            if (dsproduct != null && dsproduct.Tables.Count > 0 && dsproduct.Tables[0].Rows.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(dsproduct.Tables[0].Rows[0]["Isfreefabricswatch"].ToString()) && Convert.ToBoolean(dsproduct.Tables[0].Rows[0]["Isfreefabricswatch"].ToString()))
                {
                    ...
                    divrationmain.Attributes.Add("style", "display:none;");
                    divProductReview.Attributes.Add("style", "display:none;");
                ...
                if (!String.IsNullOrEmpty(dsproduct.Tables[0].Rows[0]["Imagename"].ToString()))
                {
                    if (File.Exists(AppLogic.AppConfigs("Live_Contant_Server_Path") + (AppLogic.AppConfigs("ImagePathProduct") + "medium/" + dsproduct.Tables[0].Rows[0]["Imagename"].ToString())))
                    {
                        ...
                        imgMain.Attributes.Add("onclick", "ShowModelHelp(" + Request.QueryString["PID"].ToString() + ");");
        ...
        private void GetnextPreviousProduct()
        {
            try
            {
                ...
                if ((Session["ProductIds"] == null || (Session["ProductIds"] != null && !Session["ProductIds"].ToString().Contains(Request.QueryString["PID"].ToString()))))
                {
                    ...
                    " INNER JOIN tb_Product ON tb_ProductCategory.ProductID =tb_Product.ProductID  " +
                    " INNER JOIN tb_Category ON tb_ProductCategory.CategoryID = tb_Category.CategoryID  " +
                    " INNER join tb_CategoryMapping On tb_Category.CategoryID= tb_CategoryMapping.CategoryID WHERE tb_Category.name not like 'SHOP%' and tb_Category.StoreID=" + Convert.ToInt32(1) + " And  tb_Product.StoreID=" + Convert.ToInt32(1) + " And tb_Category.Deleted=0 and  (tb_ProductCategory.ProductID = " + Request.QueryString["PID"] + ") ";
                    ...
                    //}
                    dsNext = CommonComponent.GetCommonDataSet("EXEC usp_Product_GetnextpreviousProduct " + Request.QueryString["PID"].ToString() + ",1," + catid + "");
