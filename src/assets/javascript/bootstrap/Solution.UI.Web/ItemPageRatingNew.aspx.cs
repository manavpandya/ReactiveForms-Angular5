...
{
    public partial class ItemPageRatingNew : System.Web.UI.Page
    {
...
FROM dbo.tb_ProductOptionsPrice INNER JOIN dbo.tb_ProductSearchType ON dbo.tb_ProductOptionsPrice.Options = dbo.tb_ProductSearchType.SearchValue WHERE tb_ProductOptionsPrice.ProductId=" + Request.QueryString["PID"].ToString() + @" Order By tb_ProductSearchType.DisplayOrder");
        ...
        private void GetProductDetails(int productID)
        {
            ...
            if (dsproduct != null && dsproduct.Tables.Count > 0 && dsproduct.Tables[0].Rows.Count > 0)
            {
                ...
                if (dsVariant != null && dsVariant.Tables.Count > 0 && dsVariant.Tables[0].Rows.Count > 0)
                {
                    ...
                    //CntReadymade = Convert.ToInt32(CommonComponent.GetScalarCommonData("Select case when ISNULL(Ismadetoready,0)=1 then 1 when ISNULL(Ismadetoorder,0)=1 then 1 else 0 end from tb_Product Where productid=" + productID + ""));
                    ...
                    Int32 CntReadymadereadymadeOrder = Convert.ToInt32(CommonComponent.GetScalarCommonData("Select case when isnull(ismadetomeasure,0)=1 then 0 else 1 end from tb_Product Where productid=" + productID + ""));
                        ...
                        if (dsVariantvalue != null && dsVariantvalue.Tables.Count > 0 && dsVariantvalue.Tables[0].Rows.Count > 0)
                        {
                            ...
                            for (int j = 0; j < dsVariantvalue.Tables[0].Rows.Count; j++)
                            {
                                ...
                                else
                                {
                                    ...
                                    radiobuttontemp += "<div style=\"float:left;width:100%;margin-bottom:5px;\" class='item-radio-display'><div class=\"iradio_flat-red\" onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantID"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\" id=\"divflat-radio-" + dsVariant.Tables[0].Rows[i]["VariantID"].ToString() + "-" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\"></div><div style=\"margin-top: 2px;\" class='item-radio-display-text'>&nbsp;" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + StrBuy1onsale.Replace("<span class=\"buy-get-free\">", "").Replace("</span>", "") + "</div></div>";
        ...
        private void LoadVideo()
        {
            try
            {
                ...
                if (System.IO.File.Exists(AppLogic.AppConfigs("Live_Contant_Server") + (imagepath)))
                { }
                else
                {
