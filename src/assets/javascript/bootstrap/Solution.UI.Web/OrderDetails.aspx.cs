    ...
    public partial class OrderDetails : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                if (Request.QueryString["ono"] != null && Request.QueryString["ono"].ToString().Length > 0)
                {
                    ...
                    if (Session["PageOlderOrder"] != null && Convert.ToString(Session["PageOlderOrder"]) == "true")
        ...
        public String AddCartItem(String OrderNo)
        {
            ...
            if (DsOrder != null && DsOrder.Tables.Count > 0 && DsOrder.Tables[0].Rows.Count > 0)
            {
                ...
                DsCartDetail = objOrder.GetCartItemForViewOldOrder(Convert.ToInt32(DsOrder.Tables[0].Rows[0]["ShoppingCardID"].ToString()), Convert.ToInt32(DsOrder.Tables[0].Rows[0]["OrderNumber"].ToString()), TempStoreID);
                ...
                for (Int32 i = 0; i < DsOrder.Tables[0].Rows.Count; i++)
                {
                    ...
                    if (DsOrder.Tables[0].Rows[0]["ShippingTrackingNumber"].ToString() != null)
                    {
                        ...
                        else
                        {
                            ...
                            dtParsalShipping1 = objOrder.GetParsalShippinginviewOldOrder(Convert.ToInt32(DsCartDetail.Tables[0].Rows[0]["ProductID"].ToString()), Convert.ToInt32(DsOrder.Tables[0].Rows[0]["OrderNumber"].ToString()), TempStoreID);
                    ...
                    else
                    {
                        if (i == 0)
                        {
                            ...
                            else
                            {
                                ...
                                TempStoreID = Convert.ToInt32(CommonComponent.GetScalarCommonData("select StoreID from tb_Order where orderNumber=" + Convert.ToInt32(DsOrder.Tables[0].Rows[0]["OrderNumber"].ToString()) + ""));
                                DataSet dtParsalShipping1 = new DataSet();
                                dtParsalShipping1 = objOrder.GetParsalShippinginviewOldOrder(Convert.ToInt32(DsCartDetail.Tables[0].Rows[0]["ProductID"].ToString()), Convert.ToInt32(DsOrder.Tables[0].Rows[0]["OrderNumber"].ToString()), TempStoreID);
