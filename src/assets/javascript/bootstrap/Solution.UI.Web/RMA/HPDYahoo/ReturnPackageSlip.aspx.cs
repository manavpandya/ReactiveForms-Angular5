...
namespace Solution.UI.Web.RMA.HPDYahoo
{
    public partial class ReturnPackageSlip : System.Web.UI.Page
    {
        ...
        private void BindCart(Int32 OrderNumber, DataSet dsOrderdata)
        {
            ...
            int OrderedCustomCartID = 0;
            int.TryParse(Request.QueryString["customcartid"].ToString(), out OrderedCustomCartID);
            DataSet dsCart = new DataSet();
            dsCart = objRMAOrder.GetOrderedShoppingCartItemsFormRetrunItem(Convert.ToInt32(dsOrderdata.Tables[0].Rows[0]["OrderNumber"].ToString()), ProductID.ToString(), OrderedCustomCartID);
