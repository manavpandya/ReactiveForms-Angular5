    ...
    public partial class Invoice : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                try
                {
                    ...
                    {
                        int OrderNumber = 0;
                        bool chkOrder = Int32.TryParse(SecurityComponent.Decrypt(Request.QueryString["ONo"].ToString()).ToString(), out OrderNumber);
                        BindInvoiceSignature(OrderNumber);
                        //Response.Write();
                        if (chkOrder)
                        {
                ...
                {
                    Response.Write(Ex.Message.ToString() + " " + Ex.StackTrace.ToString() + SecurityComponent.Decrypt(Server.UrlDecode(Request.QueryString["ONo"].ToString())));
                }
            }
        }
        ...
        private void GetOrderDetails(Int32 OrderNumber)
        {
            try
            {
                ...
                if (objDsorder != null && objDsorder.Tables.Count > 0 && objDsorder.Tables[0].Rows.Count > 0)
                {
                    ...
                    {
                        ImgStoreLogo.Src = GetConfigvalue("LIVE_SERVER",TempStoreID) + "/Images/Logo.png";
                    }
        ...
        private void BindCart(Int32 OrderNumber, DataSet dsOrderdata)
        {
            try
            {
                ...
                try
                {
                    if (!string.IsNullOrEmpty(strswatchQtyy) && Convert.ToInt32(strswatchQtyy) > Convert.ToInt32(GetConfigvalue("SwatchMaxlength",TempStoreID).ToString()) && SwatchQty == 0)
                    {
                        SwatchQty = Convert.ToInt32(strswatchQtyy) - Convert.ToInt32(GetConfigvalue("SwatchMaxlength",TempStoreID).ToString());
                    }
                }
                catch
                {
        ...
        /// </summary>
        private void BindInvoiceSignature(Int32 OrderNumber)
        {
            DataSet dsTopic = new DataSet();
            try
            {
                Int32 TempStoreID = Convert.ToInt32(CommonComponent.GetScalarCommonData("select StoreID from tb_Order where orderNumber=" + OrderNumber + ""));
                dsTopic = TopicComponent.GetTopicList("InvoiceSignature", Convert.ToInt32(GetConfigvalue("StoreID",TempStoreID)));
            }
            catch
            {
            ...
            {
                dsTopic = CommonComponent.GetCommonDataSet("SELECT 'Thank You,<br>'+StoreName as InvoiceSignature FROM dbo.tb_Store WHERE StoreID in (SELECT StoreID FROM Tb_Order WHERE OrderNumber=" + OrderNumber + ")");
                if (dsTopic != null && dsTopic.Tables.Count > 0 && dsTopic.Tables[0].Rows.Count > 0)
                {
