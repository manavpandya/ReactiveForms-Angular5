    ...
    public partial class ReturnPackageSlip : System.Web.UI.Page
    {
            ...
            if (!IsPostBack)
            {
                ...
                if (Request.QueryString["ID"] != null && Request.QueryString["ID"].ToString().Length > 0)
                {
                    ...
                    if (dsReturn != null && dsReturn.Tables[0].Rows.Count > 0)
                    {
                        ...
                        Int32 TempStoreID = Convert.ToInt32(CommonComponent.GetScalarCommonData("select StoreID from tb_Order where orderNumber=" + OrderNo + ""));
                        //AppConfig.StoreID = Convert.ToInt32(dsReturn.Tables[0].Rows[0]["StoreID"].ToString());
                        string liveservername = GetConfigvalue("LIVE_SERVER_NAME",TempStoreID);
        ...
        private void GetOrderDetails(Int32 OrderNumber)
        {
            ...
            if (objDsorder != null && objDsorder.Tables.Count > 0 && objDsorder.Tables[0].Rows.Count > 0)
            {
                ...
                {
                    //Bind Origin Address
                    strFromAddress = GetConfigvalue("Shipping.OriginContactName", TempStoreID) + "<br/>"
                   + GetConfigvalue("Shipping.OriginAddress", TempStoreID) + "<br/>";
                    if (!String.IsNullOrEmpty(GetConfigvalue("Shipping.OriginAddress2", TempStoreID)))
                        strFromAddress += GetConfigvalue("Shipping.OriginAddress2", TempStoreID) + "<br/>";
                    strFromAddress += GetConfigvalue("Shipping.OriginCity", TempStoreID) + ", " + GetConfigvalue("Shipping.OriginState", TempStoreID)
                        + "<br/>" + GetConfigvalue("Shipping.OriginZip", TempStoreID);
                }
               ...
               // ltrpaymentMethod.Text = objDsorder.Tables[0].Rows[0]["PaymentMethod"].ToString();
                ...
                BindCart(Convert.ToInt32(0), objDsorder);
            }
        }
        ...
        private void BindCart(Int32 OrderNumber, DataSet dsOrderdata)
        {
            ...
            try
            {
                ...
                if (dsCart != null && dsCart.Tables.Count > 0 && dsCart.Tables[0].Rows.Count > 0)
                {
                    ...
                    for (int i = 0; i < dsCart.Tables[0].Rows.Count; i++)
                    {
                        ...
                        if (!string.IsNullOrEmpty(dsCart.Tables[0].Rows[i]["ProductName"].ToString()))
                        {
                        ...
                        if (!string.IsNullOrEmpty(dsCart.Tables[0].Rows[i]["VariantNames"].ToString()) && !string.IsNullOrEmpty(dsCart.Tables[0].Rows[i]["VariantValues"].ToString()))
                        {
                        ...
                        if (!string.IsNullOrEmpty(dsCart.Tables[0].Rows[i]["SKU"].ToString()))
                        {
                        ...
                        if (!string.IsNullOrEmpty(dsCart.Tables[0].Rows[i]["Price"].ToString()))
                        {
            ...
            catch (Exception ex)
            {
                ...
                Response.Write("bindcart " + ex.Message.ToString() + " " + ex.StackTrace.ToString());
