...
namespace Solution.UI.Web.RMA.OverStock
{
    public partial class ViewRMARecentOrders : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            GetOrderDetails();
            lnkBack.HRef = "/rma/overstock/returnitem.aspx";
        }
        ...
        private void GetOrderDetails()
        {
          ...
          //  DataSet DsOrderPaging = new DataSet();
            AppConfig.StoreID = Convert.ToInt32(CommonComponent.GetScalarCommonData("SELECT StoreId FROM tb_Customer WHERE CustomerId=" + custid + ""));
            ...
            if (DsOrder != null && DsOrder.Tables.Count > 0 && DsOrder.Tables[0].Rows.Count > 0)
            {
                ...
                if (dr != null && dr.Length > 0)
                {
                    ...
                    foreach (DataRow drOrderNums in dr)
                    {
                        ...
                        Int32 TempStoreID = Convert.ToInt32(CommonComponent.GetScalarCommonData("select StoreID from tb_Order where orderNumber=" + OrderNumber + ""));
                        ...
                        DataSet dsOrder = new DataSet();
                        dsOrder = objOrder.GetViewRecentForRMAOrderByOrderID(Convert.ToInt32(drOrderNums["OrderNumber"].ToString()), TempStoreID);
                        if (dsOrder != null && dsOrder.Tables.Count > 0 && dsOrder.Tables[0].Rows.Count > 0)
                        {
                            ...
                            DsCartDetail = objOrder.GetCartItemForViewRecentOrder(Convert.ToInt32(dsOrder.Tables[0].Rows[0]["ShoppingCardID"].ToString()), Convert.ToInt32(dsOrder.Tables[0].Rows[0]["OrderNumber"].ToString()), TempStoreID);
                            if (DsCartDetail != null && DsCartDetail.Tables.Count > 0 && DsCartDetail.Tables[0].Rows.Count > 0)
                            {
                                ...
                                for (Int32 i = 0; i < DsCartDetail.Tables[0].Rows.Count; i++)
                                {
                                    ...
                                    string CustomCartID = Convert.ToString(DsCartDetail.Tables[0].Rows[i]["CustomCartID"].ToString());
                                    Table.Append("<tr style='vAlign:top;'>");
                                    Table.Append("<td>&nbsp;</td>");
                                    ...
                                    dsReturnItem = CommonComponent.GetCommonDataSet("select * from dbo.tb_ReturnItem where OrderedNumber=" + OrderNumber + " and ProductID=" + ProductId + " and OrderedCustomCartID=" + CustomCartID + "");
                                    ...
                                    dsShippReturnItem = CommonComponent.GetCommonDataSet("select RefProductID from dbo.tb_OrderedShoppingCartItems where RefProductID=" + DsCartDetail.Tables[0].Rows[i]["RefProductID"].ToString() + " and OrderedCustomCartID=" + CustomCartID + "");
                                   ...
                                   // dtParsalShipping1 = CommonComponent.GetCommonDataSet("select ISNULL(TrackingNumber,'') as TrackingNumber,ShippedVia,ShippedOn  from tb_OrderedShoppingCartItems where RefProductID=" + ProductId + " and OrderedCustomCartID=" + CustomCartID + " and ISNULL(TrackingNumber,'') <> '' and ISNULL(ShippedVia,'') <> '' ");
                                    ...
                                    if (!string.IsNullOrEmpty(dsOrder.Tables[0].Rows[0]["ShippingTrackingNumber"].ToString()))
                                    {
                                        ...
                                        if (!string.IsNullOrEmpty(dsOrder.Tables[0].Rows[0]["shippedon"].ToString()))
                                        {
                                            ...
                                            {
                                                DateTime dtShippedOn = Convert.ToDateTime(dtParsalShipping1.Tables[0].Rows[0]["shippedon"].ToString());
                                                String strCurrentDate = dtShippedOn.ToShortDateString();
                                                String strCurrentTrackingNo = dtParsalShipping1.Tables[0].Rows[0]["TrackingNumber"].ToString();
                                                ...
                                                if (!String.IsNullOrEmpty(dtParsalShipping1.Tables[0].Rows[0]["shippedvia"].ToString()))
                                                    Table.Append("Shipsped Via:" + dtParsalShipping1.Tables[0].Rows[0]["shippedvia"].ToString() + "<br/>");
                                            }
                                            else
                                            {
                                                isShipped = false;
                                            ...
                                            if (isReturnitem == true) // coding for RMA
                                            {
                                                ...
                                                Table.Append("RMA No: <b Style='font-size:12px;color:#D22D4F;'> RMA - " + RMANo.ToString() + "</b>  &nbsp;");
                                                //  Table.Append("for RMA slip <b Style='font-size:12px;color:#D22D4F;'><a onclick='javascript:window.open(\"ReturnPackageSlip.aspx?ID=" + RMANo.ToString() + "\", \"\",\"height=600,width=820,scrollbars=1,left=50,top=50,toolbar=no,menubar=no\");' style='text-decoration:none;cursor:pointer;'>click here</a></b><br/>");
                                                Table.Append("for RMA slip <b Style='font-size:12px;color:#D22D4F;'><a onclick='ShowModelForNotification(\"ReturnPackageSlip.aspx?ID=" + RMANo.ToString() + "\");' style='text-decoration:none;cursor:pointer;'>click here</a></b><br/>");
                                            }
                                            Table.Append("</td>");
                                            Table.Append("</tr>");
                                        }
                                        else
                                        {
                                            ...
                                            //DataSet dtParsalShipping1 = new DataSet();
                                            ////dtParsalShipping1 = CommonComponent.GetCommonDataSet("select ISNULL(TrackingNumber,'') as TrackingNumber,ShippedVia,Shipped,ShippedOn  from tb_orderShippedItems where orderNumber=" + OrderNumber + " And RefProductID=" + ProductId + " and ISNULL(TrackingNumber,'') <> '' and ISNULL(ShippedVia,'') <> '' ");
                                            //dtParsalShipping1 = CommonComponent.GetCommonDataSet("select ISNULL(TrackingNumber,'') as TrackingNumber,ShippedVia,Shipped,ShippedOn  from tb_OrderedShoppingCartItems where RefProductID=" + ProductId + " and OrderedCustomCartID=" + CustomCartID + " and ISNULL(TrackingNumber,'') <> '' and ISNULL(ShippedVia,'') <> '' ");
                                            if (dtParsalShipping1 != null && dtParsalShipping1.Tables.Count > 0 && dtParsalShipping1.Tables[0].Rows.Count > 0)
                                            {
                                                ...
                                                if (isReturnitem == true)
                                                {
                                                    ...
                                                    Table.Append("RMA No: <b Style='font-size:12px;color:#D22D4F;'> RMA - " + RMANo.ToString() + "</b>  &nbsp;");
                                                    //Table.Append("for RMA slip <b Style='font-size:12px;color:#D22D4F;'><a onclick='javascript:window.open(\"ReturnPackageSlip.aspx?ID=" + RMANo.ToString() + "\", \"\",\"height=600,width=820,scrollbars=1,left=50,top=50,toolbar=no,menubar=no\");' style='text-decoration:none;cursor:pointer;'>click here</a></b><br/>");
                                                    Table.Append("for RMA slip <b Style='font-size:12px;color:#D22D4F;'><a onclick='ShowModelForNotification(\"ReturnPackageSlip.aspx?ID=" + RMANo.ToString() + "\");'>click here</a></b><br/>");
                                                }
                                                isShipped = true;
                                                Table.Append("</td>");
                                                Table.Append("</tr>");
                                        ...
                                        else
                                        {
                                            ...
                                            if (days == 0)
                                            {
                                                if (AppConfig.StoreID > 0)
                                                {
                                                    try
                                                    {
                                                        Int32.TryParse(GetConfigvalue("ReturnDays",TempStoreID).ToString(), out days);
                                                    }
                                                    catch { }
                                                }
                                            }
                                            if (isShipped)
                                            {
                                                ...
                                                {
                                                    int IsItemalreadyretured = Convert.ToInt32(CommonComponent.GetScalarCommonData("Select ISNULL(COUNT(*),0) as TotCnt from tb_ReturnItem where OrderedNumber =" + OrderNumber.ToString() + " and ProductID=" + DsCartDetail.Tables[0].Rows[i]["RefProductID"].ToString() + " and OrderedCustomCartID=" + CustomCartID + ""));
                                                    if (IsItemalreadyretured == 0)
                                                    {
                                                        Table.Append("<a href=\"returnmerchandise.aspx?customcartid=" + Convert.ToString(DsCartDetail.Tables[0].Rows[i]["CustomCartID"].ToString()) + "&prodid=" + Convert.ToString(DsCartDetail.Tables[0].Rows[i]["RefProductID"].ToString()) + "&ono=" + Server.UrlEncode(SecurityComponent.Encrypt(drOrderNums["OrderNumber"].ToString())) + "\"><img src=\"/images/return_item.png\" /></a>&nbsp;");
                                                    }
                                                ...
                                                {
                                                    Table.Append("<a href=\"ReturnPackageSlip.aspx?customcartid=" + Convert.ToString(DsCartDetail.Tables[0].Rows[i]["CustomCartID"].ToString()) + "&prodid=" + Convert.ToString(DsCartDetail.Tables[0].Rows[i]["RefProductID"].ToString()) + "&ono=" + Server.UrlEncode(SecurityComponent.Encrypt(drOrderNums["OrderNumber"].ToString())) + "&StoreCredit=1&Qty=" + DsCartDetail.Tables[0].Rows[i]["Quantity"].ToString() + "\"><img src=\"/images/store_credit.png\" /></a>&nbsp;");
                                                }
                                                else
                                                {
                                                    Table.Append("<a style='font-weight:bold;' href=\"mailto:" + GetConfigvalue("ContactMail_ToAddress",TempStoreID).ToString() + "\"><img src=\"/images/send_e_Mail_for_request.png\" border=\"0\" /></a>&nbsp;");
                                                }
                                                Table.Append("</td>");
                                                Table.Append("</tr>");
                                            }
        ...
        public String GetMicroImage(String img,Int32 StoreID)
        {
            clsvariables.LoadAllPath();
            String[] AllowedExtensions = GetConfigvalue("AllowedExtensions", StoreID).Split(',');
            String imagepath = String.Empty;
            Random rd = new Random();
            imagepath = GetConfigvalue("ImagePathProduct", StoreID) + "Micro/" + img;
            if (img != "")
            {
            ...
            {
                return string.Concat(GetConfigvalue("ImagePathProduct", StoreID) + "Micro/image_not_available.jpg");
            }
