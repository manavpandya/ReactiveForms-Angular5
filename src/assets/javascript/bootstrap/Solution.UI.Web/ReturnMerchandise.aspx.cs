    ...
    public partial class ReturnMerchandise : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    if (Request.QueryString["ProdID"] != null && Request.QueryString["customcartid"] != null && !IsPostBack && Request.QueryString["ONo"] != null)
                    {
                        OrderComponent objOrder = new OrderComponent();
                        rbtReason1.Checked = true;
                        int ProductId = 0;
                        int OrderNumber = 0, CustomCartId = 0;
                        ...
                        try
                        {
                            ...
                            Int32.TryParse(SecurityComponent.Decrypt(Request.QueryString["ONo"].ToString()).ToString(), out OrderNumber);
                        }
                        catch { }
                        ltrVariant.Text = "";
                        if (ProductId > 0 && OrderNumber > 0)
                        {
                            ...
                            string StrQuery = "SELECT TOP (100) PERCENT tb_OrderedShoppingCartItems.OrderedCustomCartID,tb_OrderedShoppingCartItems.OrderedShoppingCartID,ISNULL(dbo.tb_Product.ReturnPolicy, '') AS ReturnPolicy, dbo.tb_Product.Name, dbo.tb_Product.DistributorID, dbo.tb_Product.ImageName,  " +
                                ...
                                " WHERE (dbo.tb_Product.ProductID = " + ProductId + ") and  tb_OrderedShoppingCartItems.OrderedCustomCartID=" + CustomCartId + " and dbo.tb_OrderedShoppingCartItems.RefProductID NOT IN " +
                            ...
                            if (dsProductReturn != null && dsProductReturn.Tables.Count > 0 && dsProductReturn.Tables[0].Rows.Count > 0)
                            {
        ...
        public void SendMail(string TOAddress)
        {
            try
            {
                ...
                {
                    itemreturn = "<tr><td style='width: 50%;font-size:12px;font-family:Arial,Helvetica,sans-serif;' class='cell'>" + lblProductName.Text.Trim() + "<br / >" + ltrVariant.Text.Trim() + "</td> " +
                ...
                if (chkItem1.Checked)
                {
                    itemreturn = "<tr><td style='width: 50%;font-size:12px;font-family:Arial,Helvetica,sans-serif;' class='cell'>" + txtItemReturned1.Text + "<br / >" + ltrVariant.Text.Trim() + "</td> " +
                ...
                sw.Append("<img style='border-width:0px;' src=\"" + liveserver + "/images/logo.png\"/>");
                sw.Append("</a></td>");
                sw.Append("</tr>");
                ...
                sw.Append(" <a class='content_font_orange' href='http://" + liveserver.ToString().ToLower() + "'>" + liveserver.ToString() + "</a>");
                sw.Append("</td>");
                sw.Append("</tr>");
                ...
                int CustomCartId = 0;
                Int32.TryParse(Request.QueryString["customcartid"].ToString(), out CustomCartId);
                if (CustomCartId > 0 && CustID != 0)
                {
                    if (CustID.ToString() == ViewState["CustomerID"].ToString())
                    {
                        if (CheckProductAndQuantity(CustomCartId))
                        {
                            if (Request.QueryString["ProdID"] != null)
                            {
                                if (InsertRMADeatils(CustomCartId) == false)
                                    return;
                            }
                            else
                            {
                                InsertReturnMerchandise(CustomCartId);
                            }
        ...
        #region Check Product And Quantity

        /// <summary>
        /// Check and Insert the Details into Table
        /// </summary>
        /// <returns>Returns True or False</returns>
        protected Boolean CheckProductAndQuantity(int CustomCartId)
        {
            try
            {
                int Pr1 = 0;
                ...
                if (!String.IsNullOrEmpty(txtItemReturned1.Text.ToString()) && !String.IsNullOrEmpty(txtItemCode1.Text.ToString()) && !String.IsNullOrEmpty(txtQuantity1.Text.ToString()))
                {
                    ...
                    else
                    {
                        ...
                        if (flag)
                        {
                            ...
                            {
                                qu1 = CheckQuantityProduct(Convert.ToInt32(txtOrderNumber.Text.ToString()), Convert.ToInt32(Pr1), CustomCartId);
                ...
                if (!String.IsNullOrEmpty(txtItemReturned2.Text.ToString()) && !String.IsNullOrEmpty(txtItemCode2.Text.ToString()) && !String.IsNullOrEmpty(txtQuantity2.Text.ToString()))
                {
                    ...
                    else
                    {
                        ...
                        if (flag)
                        {
                            ...
                            {
                                qu2 = CheckQuantityProduct(Convert.ToInt32(txtOrderNumber.Text.ToString()), Convert.ToInt32(Pr2), CustomCartId);
                ...
                if (!String.IsNullOrEmpty(txtItemReturned3.Text.ToString()) && !String.IsNullOrEmpty(txtItemCode3.Text.ToString()) && !String.IsNullOrEmpty(txtQuantity3.Text.ToString()))
                {
                    ...
                    else
                    {
                        ...
                        if (flag)
                        {
                            ...
                            {
                                qu3 = CheckQuantityProduct(Convert.ToInt32(txtOrderNumber.Text.ToString()), Convert.ToInt32(Pr3), CustomCartId);
                ...
                if (!String.IsNullOrEmpty(txtItemReturned4.Text.ToString()) && !String.IsNullOrEmpty(txtItemCode4.Text.ToString()) && !String.IsNullOrEmpty(txtQuantity4.Text.ToString()))
                {
                    ...
                    else
                    {
                        ...
                        if (flag)
                        {
                            ...
                            {
                                qu4 = CheckQuantityProduct(Convert.ToInt32(txtOrderNumber.Text.ToString()), Convert.ToInt32(Pr4), CustomCartId);
        ...
        public Int32 GetProductID(string ProductName, String SKU)
        {
            try
            {
                ...
                {
                    String Query = "select top 1 ProductID from tb_Product where storeid=" + ViewState["StoreId"].ToString() + " and isnull(active,0)=1 and isnull(deleted,0)=0 and SKU='" + SKU + "'";
        ...
        /// <summary>
        /// Inserts the Return Merchandise
        /// </summary>
        /// <returns>Returns True if Inserted else False</returns>
        protected Boolean InsertReturnMerchandise(int CustomCartId)
        {
            try
            {
                int Pr1 = 0;
        ...
        /// </summary>
        /// <returns>Returns True if Inserted else False</returns>
        private Boolean InsertRMADeatils(int CustomCartID)
        {
            //By Anil
            if (CustomCartID > 0 && !String.IsNullOrEmpty(lblProductName.Text.ToString()) && !String.IsNullOrEmpty(lblProductSKU.Text.ToString()) && !String.IsNullOrEmpty(TxtProductQty.Text.ToString()))
            {
                Int32 PID = 0;
                ...
                else
                {
                    ...
                    if (flag)
                    {
                        ...
                        if (!string.IsNullOrEmpty(TxtProductQty.Text))
                            qu1 = CheckQuantityProduct(Convert.ToInt32(txtOrderNumber.Text.ToString()), Convert.ToInt32(PID), CustomCartID);
                        ...
                        objReturnItem.OrderedCustomCartID = Convert.ToInt32(CustomCartID);
        ...
        /// <summary>
        /// Checks for Exists Product  for Particular Customer and Order Number
        /// </summary>
        /// <param name="ProductID">int ProductID</param>
        /// <param name="CustomerID">int CustomerID</param>
        /// <param name="OrderNo">int OrderNo</param>
        /// <returns>Returns No. of Count of Product</returns>
        public Int32 CheckForExistsProduct(Int32 ProductID, Int32 CustomerID, Int32 OrderNo)
        {
            ...
            if (Request.QueryString["customcartid"] != null)
            {
                ...
                if (CustomerID == 0)
                    Query = "select count(*) As Count from tb_ReturnItem where ProductID=" + ProductID + " and OrderedNumber=" + OrderNo + " and OrderedCustomCartID=" + CustomCartId + " and Deleted=0";
                else
                    Query = "select count(*) As Count from tb_ReturnItem where ProductID=" + ProductID + " and OrderedCustomerID=" + CustomerID + " and OrderedCustomCartID=" + CustomCartId + " and OrderedNumber=" + OrderNo + " and Deleted=0";
        ...
        /// <param name="ProductID">int ProductID</param>
        /// <returns>Returns the Count of Product Quantity</returns>
        public Int32 CheckQuantityProduct(int OrderNumber, int ProductID, int CustomCartID)
        {
            int qu = (Convert.ToInt32(CommonComponent.GetScalarCommonData(" select Count(Quantity) from dbo.tb_OrderedShoppingCartItems where OrderedShoppingCartID = " +
                   " (SELECT ShoppingCardID FROM tb_Order WHERE OrderNumber=" + OrderNumber + ") and RefProductID=" + ProductID + " and OrderedCustomCartID=" + CustomCartID + "")));
            if (qu == 0)
                return 0;
            else
                return (Convert.ToInt32(CommonComponent.GetScalarCommonData(" select ISNULL(Quantity,0) from dbo.tb_OrderedShoppingCartItems where OrderedShoppingCartID = " +
                   " (SELECT ShoppingCardID FROM tb_Order WHERE OrderNumber=" + OrderNumber + ") and RefProductID=" + ProductID + " and OrderedCustomCartID=" + CustomCartID + "")));
        }
        ...
        protected void btnsubmit_Click(object sender, ImageClickEventArgs e)
        {
            try
            {
                if (chkreturnpolicy.Checked)
                {
                    ...
                    Int32 Qty = 0;
                    int CustomCartId = 0;
                    int.TryParse(Request.QueryString["customcartid"].ToString(), out CustomCartId);
                    if (hdnProductId.Value != "")
                        Qty = Convert.ToInt32(CommonComponent.GetScalarCommonData("Select (Select Sum(tb_OrderedShoppingCartItems.Quantity) from tb_OrderedShoppingCartItems WHERE (dbo.tb_OrderedShoppingCartItems.OrderedShoppingCartID IN (SELECT ShoppingCardID FROM  dbo.tb_Order  WHERE (OrderNumber = " + SecurityComponent.Decrypt(Convert.ToString(Request.QueryString["ono"])) + ")))  AND (dbo.tb_OrderedShoppingCartItems.RefProductID = " + hdnProductId.Value + ") and tb_OrderedShoppingCartItems.OrderedCustomCartID=" + CustomCartId + ")-(select IsNULL(Sum(tb_ReturnItem.Quantity),0) from dbo.tb_ReturnItem where OrderedNumber=" + SecurityComponent.Decrypt(Convert.ToString(Request.QueryString["ono"])) + " and ProductID=" + hdnProductId.Value + " and OrderedCustomCartID=" + CustomCartId + ")"));
                    ...
                    if (!string.IsNullOrEmpty(TxtProductQty.Text.ToString()) && Convert.ToInt32(TxtProductQty.Text) > 0)
                    {
                        ...
                        if (MailAddress != string.Empty)
                        {
                            ...
                            if (flagreturn == true || ReturnID == 0)
                                ...
                                try
                                {
                                    Response.Redirect("ReturnPackageSlip.aspx?Page=ReturnMerchandise&ID=" + ReturnID + "&customcartid=" + CustomCartId.ToString() + "");
                                }
                                catch { }
                        }
                        else
