...
{
    public partial class Roman_ItemPage : System.Web.UI.Page
    {
        ...
        private void GetProductDetails(int productID)
        {
            ...
            if (dsproduct != null && dsproduct.Tables.Count > 0 && dsproduct.Tables[0].Rows.Count > 0)
            {
                ...
                if (!string.IsNullOrEmpty(dsproduct.Tables[0].Rows[0]["Coloroption"].ToString()))
                {
                    ...
                    if (dsColor != null && dsColor.Tables.Count > 0 && dsColor.Tables[0].Rows.Count > 0)
                    {
                        ...
                        for (int i = 0; i < dsColor.Tables[0].Rows.Count; i++)
                        {
                            ...
                            string strdic = "<div style='float:left;width:100%;'><a href='" + strhref + "' style='cursor:pointer'><img src='" + AppLogic.AppConfigs("Live_Contant_Server") + strImagePath.Replace("/micro/", "/icon/") + "' /></a></div><div class='divtooltipover'><a href='" + strhref + "' style='cursor:pointer'>" + dsColor.Tables[0].Rows[i]["name"].ToString() + "</a></div>";
                            strResult += "<li><a href=\"" + strhref + "\" id=\"aimgcolor" + i.ToString() + "\" style=\"cursor:" + strstyle + ";\"><img  src=\"" + AppLogic.AppConfigs("Live_Contant_Server") + strImagePath.Trim() + "\" alt=\"" + Convert.ToString(dsColor.Tables[0].Rows[i]["name"]) + "\" title=\"" + strdic + "\" style=\"border:solid 1px #dddddd;height:66px;width:66px;\"></a></li>";
                            if (i == 0)
                            {
                ...
                if (!string.IsNullOrEmpty(dsproduct.Tables[0].Rows[0]["Ismadetomeasure"].ToString()) && Convert.ToBoolean(dsproduct.Tables[0].Rows[0]["Ismadetomeasure"].ToString()))
                {
                    ...
                    areadymade.InnerHtml = "Custom Size";
                    areadymade.Title = "Custom Size";
                }
                ...
                if (String.IsNullOrEmpty(dsproduct.Tables[0].Rows[0]["TabDesc2"].ToString()))
                {
                    divspecification.Visible = false;
                    divspecifications.Visible = false;
                    divspecifications1.Visible = false;
                }
                else
                {
                    divspecification.Visible = true;
                    divspecifications.Visible = true;
                    divspecifications1.Visible = true;
                    ltProductSpecifications.Text = Convert.ToString(dsproduct.Tables[0].Rows[0]["TabDesc2"].ToString());
                }
                ...
                if (!String.IsNullOrEmpty(dsproduct.Tables[0].Rows[0]["Imagename"].ToString()))
                {
                    if (File.Exists(AppLogic.AppConfigs("Live_Contant_Server_Path") + (AppLogic.AppConfigs("ImagePathProduct") + "medium/" + dsproduct.Tables[0].Rows[0]["Imagename"].ToString())))
                    {
                        ...
                        imgMain.Attributes.Add("onclick", "ShowModelHelp(" + productID.ToString() + ");");
                    ...
                    else
                    {
                        ...
                        imgMain.Src = AppLogic.AppConfigs("Live_Contant_Server") + AppLogic.AppConfigs("ImagePathProduct") + "medium/image_not_available.jpg?" + rd.Next(10000).ToString();
...
Where isnull(tb_ProductVariantValue.VarActive,0)=1 and tb_product.ProductId=" + Request.QueryString["PID"].ToString() + @" and isnull(tb_product.Active,0)=1 AND  isnull(tb_product.deleted,0)=0 AND isnull(tb_product.storeid,0)=1 AND (cast(Buy1Fromdate as date) <=  cast(GETDATE() as date) and cast(Buy1Todate as date) >=cast(GETDATE() as date)) and ISNULL(Buy1Get1,0)=1)"));

                if (countptod > 0)
                {
                    lblTag.Text += "<img title='" + StrTagName.ToString().Trim() + "' src=\"/images/bogof.png\" alt=\"" + StrTagName.ToString().Trim() + "\" class='bu1getitem' />";
                }
                // Video by LPR
                ...
                if (dsVariant != null && dsVariant.Tables.Count > 0 && dsVariant.Tables[0].Rows.Count > 0)
                {
                   ...
                   // strmeasurement += "<span id=\"spancolspan-0\">3</span>SELECT MEASUREMENTS";
                    if (ViewState["isRomanShade"] != null && ViewState["isRomanShade"].ToString() == "1")
                    {


                        strmeasurement += "<span id=\"spancolspan-0\">2</span>SELECT MEASUREMENTS";
                    }
                    else
                    {
                        strmeasurement += "<span id=\"spancolspan-0\">3</span>SELECT MEASUREMENTS";
                    }
                 strmeasurement += " <div style=\"float:right;line-height:25px;padding-right:2px;padding-top: 2px;\" id=\"divselvalue-0\"><a title=\"Learn More\" href=\"javascript:void(0);\"  style=\"color:#B92127;\" onclick=\"ShowModelForMeasuringguide();\">Learn More</a>";
                   // strmeasurement += "<div style=\"float: right; line-height: 25px; padding-right: 2px;\" id=\"divselvalue-0\">";
                    ...
                    if (ViewState["isRomanShade"] != null && ViewState["isRomanShade"].ToString() == "1")
                    {
                        DataSet dsWidth = CommonComponent.GetCommonDataSet("SELECT * FROM tb_Shadewidth WHERE ProductId=" + Request.QueryString["PID"].ToString() + "");
                        if (dsWidth != null && dsWidth.Tables.Count > 0 && dsWidth.Tables[0].Rows.Count > 0)
                        {
                            for (int w = 0; w < dsWidth.Tables[0].Rows.Count; w++)
                            {
                                strmeasurement += "<option value=\"" + string.Format("{0:0}", Convert.ToDouble(dsWidth.Tables[0].Rows[w]["value"].ToString())) + "\">" + string.Format("{0:0}", Convert.ToDouble(dsWidth.Tables[0].Rows[w]["value"].ToString())) + "</option>";
                            }
                        }
                    ...
                    if (ViewState["isRomanShade"] != null && ViewState["isRomanShade"].ToString() == "1")
                    {
                        DataSet dsLength = CommonComponent.GetCommonDataSet("SELECT * FROM tb_ShadeLength WHERE ProductId=" + Request.QueryString["PID"].ToString() + "");
                  ...
                  //  strmeasurement += "<div style=\"float: right; width: 95%; text-align: right;\"><img src=\"/images/next-option.png\" alt=\"Next\" title=\"Next\" border=\"0\" style=\"cursor:pointer;\" onclick=\"if(document.getElementById('Selectvariant-0') != null && document.getElementById('Selectvariant-0').selectedIndex == 0){jAlert('Please select width.','Message');}else if(document.getElementById('Selectvariant-99999999') != null && document.getElementById('Selectvariant-99999999').selectedIndex == 0){jAlert('Please select length.','Message');}else{varianttabhideshow(1000000);}\" /></div>";
                    ...
                    DataSet dsstyle = new DataSet();
                    dsstyle = CommonComponent.GetCommonDataSet(@"SELECT case when isnull(tb_ProductOptionsPrice.AdditionalPrice,0)=0 then tb_ProductOptionsPrice.Options else tb_ProductOptionsPrice.Options+'($'+cast(Round(tb_ProductOptionsPrice.AdditionalPrice,2) as varchar(10))+')'  end as name, tb_ProductOptionsPrice.ProductId, tb_ProductSearchType.DisplayOrder
FROM dbo.tb_ProductOptionsPrice INNER JOIN dbo.tb_ProductSearchType ON dbo.tb_ProductOptionsPrice.Options = dbo.tb_ProductSearchType.SearchValue WHERE isnull(tb_ProductOptionsPrice.Active,0)=1 AND tb_ProductOptionsPrice.ProductId=" + Request.QueryString["PID"].ToString() + @" Order By tb_ProductSearchType.DisplayOrder");
                    ...
                    if (dsstyle != null && dsstyle.Tables.Count > 0 && dsstyle.Tables[0].Rows.Count > 0)
                    {
                        ...
                        {
                            stroptions += "<option value=\"100000" + (iOption + 1).ToString() + "\">" + Server.HtmlEncode(dsstyle.Tables[0].Rows[iOption]["name"].ToString()) + "</option>";
                        }
                        stroptions += "</select>";
                        for (int iOption = 0; iOption < dsstyle.Tables[0].Rows.Count; iOption++)
                        {
                            stroptions += "<div style=\"float: left; width: 100%; margin-bottom: 5px;\">";
                            stroptions += "<div id=\"divflat-radio-1000000-100000" + (iOption + 1).ToString() + "\" onclick=\"selecteddropdownvalue(1000000,100000" + (iOption + 1).ToString() + ");PriceChangeondropdown();\" class=\"iradio_flat-red\">";
                            stroptions += "</div>";
                            stroptions += "<div style=\"margin-top: 2px;\" class='item-radio-display-text' onclick=\"selecteddropdownvalue(1000000,100000" + (iOption + 1).ToString() + ");PriceChangeondropdown();\"> &nbsp;" + Server.HtmlEncode(dsstyle.Tables[0].Rows[iOption]["name"].ToString()) + "</div>";
                            stroptions += "</div>";
                            checkedshade = true;
                        }
                    ...
                    else
                    {
                        ...
                        stroptions += "</div>";
                        stroptions += "<div style=\"margin-top: 2px;\" class='item-radio-display-text' onclick=\"selecteddropdownvalue(1000000,1000001);PriceChangeondropdown();\"> &nbsp;Lined</div>";
                        stroptions += "</div>";
                        ...
                        stroptions += "</div>";
                        stroptions += "<div style=\"margin-top: 2px;\" class='item-radio-display-text' onclick=\"selecteddropdownvalue(1000000,1000002);PriceChangeondropdown();\"> &nbsp;Lined &amp; Interlined</div>";
                        stroptions += "</div>";
                        ...
                        stroptions += "</div>";
                        stroptions += "<div style=\"margin-top: 2px;\" class='item-radio-display-text' onclick=\"selecteddropdownvalue(1000000,1000003);PriceChangeondropdown();\"> &nbsp;Blackout Lining</div>";
                        stroptions += "</div>";
                    ...
                    if (checkedshade == false && ViewState["isRomanShade"] != null && ViewState["isRomanShade"].ToString() == "1")
                    {
                        stroptions = "";
                    }
                  ...
                  //  strmount += "<span id=\"spancolspan-2000000\">5</span>Mount</div>";
                    if (checkedshade == false && ViewState["isRomanShade"] != null && ViewState["isRomanShade"].ToString() == "1")
                    {
                        strmount += "<span id=\"spancolspan-2000000\">3</span>Mount</div>";
                    }
                    else
                    {
                        strmount += "<span id=\"spancolspan-2000000\">5</span>Mount</div>";
                    ...
                    strmount += "<div style=\"float: left; width: 50%; margin-bottom: 5px;\" onclick=\"selecteddropdownvalue(2000000,2000001);PriceChangeondropdown();\">";
                    ...
                    strmount += "<div style=\"float: left; width: 50%; margin-bottom: 5px;\" onclick=\"selecteddropdownvalue(2000000,2000002);PriceChangeondropdown();\">";
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                if ((dsVariant.Tables[0].Rows[i]["VariantName"].ToString().ToLower().IndexOf("cord") > -1))
                                {
                                    ...
                                    if (ViewState["isRomanShade"] != null && ViewState["isRomanShade"].ToString() == "1")
                                    {
                                        strtttCord += "<span id=\"spancolspan-" + dsVariant.Tables[0].Rows[i]["VariantID"].ToString() + "\">" + iNumb.ToString() + "</span>" + dsVariant.Tables[0].Rows[i]["VariantName"].ToString();
                                    }
                                    else
                                    {
                                        strtttCord += "<span id=\"spancolspan-" + dsVariant.Tables[0].Rows[i]["VariantID"].ToString() + "\">6</span>" + dsVariant.Tables[0].Rows[i]["VariantName"].ToString();
                        ...
                        if (dsVariantvalue != null && dsVariantvalue.Tables.Count > 0 && dsVariantvalue.Tables[0].Rows.Count > 0)
                        {
                            ...
                            if (!string.IsNullOrEmpty(dsVariant.Tables[0].Rows[i]["VariantName"].ToString()) && dsVariant.Tables[0].Rows[i]["VariantName"].ToString().ToLower().IndexOf("color") > -1)
                            {
                                    ...
                                    for (int j = 0; j < dsVariantvalue.Tables[0].Rows.Count; j++)
                                    {
                                        if (!string.IsNullOrEmpty(dsVariantvalue.Tables[0].Rows[j]["VariImageName"].ToString()))
                                        {
                                            ...
                                            if (!string.IsNullOrEmpty(ColorImgName))
                                            {
                                                ...
                                                string strcolorFilePath = AppLogic.AppConfigs("Live_Contant_Server_Path") + StrColorPath + ColorImgName;
                                                if (File.Exists(strcolorFilePath))
                                                {
                                                    IsVariColor = true;
                                                    StrColorPath = AppLogic.AppConfigs("Live_Contant_Server") + StrColorPath + ColorImgName + "?" + rnd.Next(10000);
                                                    string strdic = "";
                                                    string name = "";
                                                    name = dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString();
                                                    if (name.Length > 24)
                                                    {
                                                    ...
                                                    if (Session["VID"] != null && Convert.ToString(Session["VID"]) != "" && (dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString().ToLower() == Convert.ToString(Session["VID"]).ToLower()))
                                                    {
                                                        ...
                                                        String RelatedSku = Convert.ToString(CommonComponent.GetScalarCommonData("select isnull(RelatedProductid,'') from tb_ProductVariantValue where VariantValueID=" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ""));
                                                        if (!String.IsNullOrEmpty(RelatedSku))
                                                        {
                                                            
                                                            DataSet dsrelated = CommonComponent.GetCommonDataSet("select Productid,sku,ProductURL,Name,imagename from tb_product where sku='" + RelatedSku + "' and isnull(active,0)=1 and isnull(deleted,0)=0 and storeid=1");
                                                            if (dsrelated != null && dsrelated.Tables.Count > 0 && dsrelated.Tables[0].Rows.Count > 0)
                                                            {
                                                                //string vaImagenm = "";
                                                                //if (File.Exists(AppLogic.AppConfigs("Live_Contant_Server_Path") + (AppLogic.AppConfigs("ImagePathProduct") + "icon/" + dsrelated.Tables[0].Rows[0]["imagename"].ToString())))
                                                                //{
                                                                //    vaImagenm = AppLogic.AppConfigs("Live_Contant_Server") + AppLogic.AppConfigs("ImagePathProduct") + "icon/" + dsrelated.Tables[0].Rows[0]["imagename"].ToString();
                                                                //}
                                                                //if (vaImagenm != "")
                                                                //{
                                                                //    strdic = "<div style='float:left;width:100%;'><a href='/" + dsrelated.Tables[0].Rows[0]["ProductURL"].ToString() + "' style='cursor:pointer'><img src='" + vaImagenm + "' /></a></div><div class='divtooltipover'><a href='/" + dsrelated.Tables[0].Rows[0]["ProductURL"].ToString() + "' style='cursor:pointer'>" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + "</a>-<a href='javascript:void(0);' onclick='InsertProductSwatch(" + dsrelated.Tables[0].Rows[0]["ProductId"].ToString() + ",'ContentPlaceHolder1_btnAddTocartSwatch'); return false;' style='cursor:pointer'>BUY SWATCH</a></div>";
                                                                //}

                                                                
                                                                strdic = "<div style=\"float:left;width:100%;\"><a href=\"/" + dsrelated.Tables[0].Rows[0]["ProductURL"].ToString() + "\" style=\"cursor:pointer\"><img src=\"" + StrColorPath + "\" /></a></div><div class=\"divtooltipover\"><a href=\"/" + dsrelated.Tables[0].Rows[0]["ProductURL"].ToString() + "\" style=\"cursor:pointer\">" + name + "</a><br /><a href=\"javascript:void(0);\" onclick=\"InsertRomanProductSwatch(" + dsrelated.Tables[0].Rows[0]["ProductId"].ToString() + ",'ContentPlaceHolder1_btnAddTocartSwatch'); return false;\" style=\"cursor:pointer\">BUY SWATCH</a></div>";


                                                            }
                                                            else
                                                            {
                                                                strdic = "<div style=\"float:left;width:100%;\"><a href=\"javascript:void(0);\" style=\"cursor:pointer\"><img src=\"" + StrColorPath + "\" /></a></div><div class=\"divtooltipover\"><a href=\"javascript:void(0);\" style=\"cursor:pointer\">" + name + "</a></div>";
                                                            }
                                                        }
                                                        else
                                                        {
                                                        ...
                                                        // string strdic = "<div style='float:left;width:100%;'><a href='javascript:void(0);' style='cursor:pointer'><img src='" + StrColorPath.Replace("/color/", "/color/large/") + "' /></a></div><div class='divtooltipover'><a href='javascript:void(0);' style='cursor:pointer'>" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + "</a></div>";
                                                        //StrColorVari += "<li id=\"id_color_" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\" style=\"width:auto !important;\" onclick=\"SelectColorli(" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");\" class=\"coloractiveli\"><a href=\"javascript:void(0);\"  onclick=\"SelectVariantByColor('" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + "'," + dsVariant.Tables[0].Rows[i]["VariantID"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");\"><img id=\"img_color_" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\" title=\"" + strdic.ToString() + "\" alt=\"" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + "\" src=\"" + StrColorPath + "\"></a>";
                                                        StrColorVari += "<li id=\"id_color_" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\" style=\"width:auto !important;padding: 4px 0px 2px 15px !important;\" onclick=\"SelectColorli(" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");\" ><a href=\"javascript:void(0);\"  onclick=\"SelectVariantByColor('" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + "'," + dsVariant.Tables[0].Rows[i]["VariantID"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");\"><img id=\"img_color_" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\" title=\"" + Server.HtmlEncode(strdic) + "\" alt=\"" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + "\" src=\"" + StrColorPath + "\" style=\"border:solid 1px #828282;\"></a>";
                                                    }
                                                    else
                                                    {
                                                        String RelatedSku = Convert.ToString(CommonComponent.GetScalarCommonData("select isnull(RelatedProductid,'') from tb_ProductVariantValue where VariantValueID=" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ""));
                                                        if (!String.IsNullOrEmpty(RelatedSku))
                                                        {
                                                            ...
                                                            if (dsrelated != null && dsrelated.Tables.Count > 0 && dsrelated.Tables[0].Rows.Count > 0)
                                                            {
                                                                ...
                                                                //if (vaImagenm != "")
                                                                //{
                                                                //    strdic = "<div style='float:left;width:100%;'><a href='/" + dsrelated.Tables[0].Rows[0]["ProductURL"].ToString() + "' style='cursor:pointer'><img src='" + vaImagenm + "' /></a></div><div class='divtooltipover'><a href='/" + dsrelated.Tables[0].Rows[0]["ProductURL"].ToString() + "' style='cursor:pointer'>" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + "</a>-<a href='javascript:void(0);' onclick='InsertProductSwatch(" + dsrelated.Tables[0].Rows[0]["ProductId"].ToString() + ",'ContentPlaceHolder1_btnAddTocart'); return false;' style='cursor:pointer'>BUY SWATCH</a></div>";
                                                                //}
                                        ...
                                        if (strsc1 == "" && StrColorVari != "")
                                        {

                                            strsc1 += "$(\"#color_ul_name > li > a > img[title]\").tooltip({";
                                            strsc1 += "offset: [10, 2],";
                                            strsc1 += "effect: 'slide'";
                                            strsc1 += "}).dynamic({ bottom: { direction: 'down', bounce: true } });";
                                            strsc1 = "var $jj= jQuery.noConflict(); $(function () {" + strsc1 + "});";
                                ...
                                if (IsVariColor)
                                {
                                    ...
                                    strsccolor = "$(\"#color_ul_name > li > a > img[title]\").tooltip({";
                                    strsccolor += "offset: [10, 2],";
                                    strsccolor += "effect: 'slide'";
                                    strsccolor += "}).dynamic({ bottom: { direction: 'down', bounce: true } });";

                                    strsccolor = "var $kkkk= jQuery.noConflict(); $(function () {" + strsccolor + "});";
                                    if (ViewState["isRomanShade"] != null && ViewState["isRomanShade"].ToString() == "1")
                                    {
                                        iNumb++;
                                        iNumb++;
                            ...
                            for (int j = 0; j < dsVariantvalue.Tables[0].Rows.Count; j++)
                            {
                                ...
                                else
                                {
                                    if (dsVariant.Tables[0].Rows[i]["VariantName"].ToString().ToLower().IndexOf("color") > -1)
                                    {
                                        ...
                                        strttt += "<div style=\"float:left;width:100%;margin-bottom:5px;\" class='item-radio-display' onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\"><div class=\"iradio_flat-red\" onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\" id=\"divflat-radio-" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "-" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\"></div><div style=\"margin-top: 2px;\" class='item-radio-display-text'>&nbsp;" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + StrBuy1onsale + StrbackOrderdate + "</div></div>";
                                    ...
                                    else if (dsVariant.Tables[0].Rows[i]["VariantName"].ToString().ToLower().IndexOf("cord") > -1)
                                    {
                                        ...
                                        {
                                            strtttCord += "<div style=\"float:left;width:50%;margin-bottom:5px;\" class='item-radio-display' onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\"><div style=\"float: left; width: 100%;\"><img src=\"/images/left-cord.png\" alt=\"Left\" title=\"Left\"></div><div style=\"margin-left: 17%;\"><div class=\"iradio_flat-red\" onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\" id=\"divflat-radio-" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "-" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\"></div><div style=\"margin-top: 2px;float: left;\">&nbsp;" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + StrBuy1onsale + strPrice.Replace("(", " - ").Replace(")", "") + StrbackOrderdate + "</div></div></div>";
                                        }
                                        else if (dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString().ToLower().IndexOf("right") > -1)
                                        {
                                            strtttCord += "<div style=\"float:left;width:50%;margin-bottom:5px;\" class='item-radio-display' onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\"><div style=\"float: left; width: 100%;\"><img src=\"/images/right-cord.png\" alt=\"Right\" title=\"Right\"></div><div style=\"margin-left: 17%;\"><div class=\"iradio_flat-red\" onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\" id=\"divflat-radio-" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "-" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\"></div><div style=\"margin-top: 2px;float: left;\">&nbsp;" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + StrBuy1onsale + strPrice.Replace("(", " - ").Replace(")", "") + StrbackOrderdate + "</div></div></div>";
                                        }
                                        else
                                        {
                                            strtttCord += "<div style=\"float:left;width:100%;margin-bottom:5px;\" class='item-radio-display' onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\"><div class=\"iradio_flat-red\" onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\" id=\"divflat-radio-" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "-" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\"></div><div style=\"margin-top: 2px;\" class='item-radio-display-text'>&nbsp;" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + StrBuy1onsale + strPrice.Replace("(", " - ").Replace(")", "") + StrbackOrderdate + "</div></div>";
                                        }
                                    }
                                    else
                                    {
                                        ...
                                        //    if (!string.IsNullOrEmpty(ColorImgName))
                                        //    {
                                        //        StrColorPath = string.Concat(AppLogic.AppConfigs("ImagePathProductVariant"), "Color/Large/");
                                        //        if (!Directory.Exists(Server.MapPath(StrColorPath)))
                                        //            Directory.CreateDirectory(Server.MapPath(StrColorPath));
                                        //        string strcolorFilePath = AppLogic.AppConfigs("Live_Contant_Server_Path") + StrColorPath + ColorImgName;
                                        //        if (File.Exists(strcolorFilePath))
                                        ...
                                        if (dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString().ToLower().IndexOf("casual") > -1)
                                        {
                                            //if (checkimage)
                                            //{
                                            //    strttt += "<div style=\"float:left;width:25%;margin-bottom:5px;\" class='item-radio-display'><div style=\"float: left; width: 100%;\"><img title=\"Casual\" alt=\"Casual\" src=\"/images/Casual.jpg\"></div><div style=\"margin-left: 30%;\"><div class=\"iradio_flat-red\" onclick=\"imagereplace('" + StrColorPath + "');selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\" id=\"divflat-radio-" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "-" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\"></div><div style=\"margin-top: 2px;\" class='item-radio-display-text'>&nbsp;" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + StrBuy1onsale + strPrice.Replace("(", " - ").Replace(")", "") + StrbackOrderdate + "</div></div></div>";
                                            //}
                                            //else
                                            //{
                                            strttt += "<div style=\"float:left;width:25%;margin-bottom:5px;\" class='item-radio-display' onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\"><div style=\"float: left; width: 100%;\"><img title=\"Casual\" alt=\"Casual\" src=\"/images/Casual.jpg\"></div><div style=\"margin-left: 30%;\"><div class=\"iradio_flat-red\" onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\" id=\"divflat-radio-" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "-" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\"></div><div style=\"margin-top: 2px;\" class='item-radio-display-text'>&nbsp;" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + StrBuy1onsale + strPrice.Replace("(", " - ").Replace(")", "") + StrbackOrderdate + "</div></div></div>";
                                            //}
                                        ...
                                        else if (dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString().ToLower().IndexOf("soft fold") > -1)
                                        {
                                            //if (checkimage)
                                            //{
                                            //    strttt += "<div style=\"float:left;width:25%;margin-bottom:5px;\" class='item-radio-display'><div style=\"float: left; width: 100%;\"><img title=\"Soft Fold\" alt=\"Soft Fold\" src=\"/images/Soft_Fold.jpg\"></div><div style=\"margin-left: 30%;\"><div class=\"iradio_flat-red\" onclick=\"imagereplace('" + StrColorPath + "');selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\" id=\"divflat-radio-" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "-" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\"></div><div style=\"margin-top: 2px;\" class='item-radio-display-text'>&nbsp;" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + StrBuy1onsale + strPrice.Replace("(", " - ").Replace(")", "") + StrbackOrderdate + "</div></div></div>";
                                            //}
                                            //else
                                            //{
                                            strttt += "<div style=\"float:left;width:25%;margin-bottom:5px;\" class='item-radio-display' onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\"><div style=\"float: left; width: 100%;\"><img title=\"Soft Fold\" alt=\"Soft Fold\" src=\"/images/Soft_Fold.jpg\"></div><div style=\"margin-left: 30%;\"><div class=\"iradio_flat-red\" onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\" id=\"divflat-radio-" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "-" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\"></div><div style=\"margin-top: 2px;\" class='item-radio-display-text'>&nbsp;" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + StrBuy1onsale + strPrice.Replace("(", " - ").Replace(")", "") + StrbackOrderdate + "</div></div></div>";
                                            //}
                                        ...
                                        else if (dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString().ToLower().IndexOf("relaxed") > -1)
                                        {
                                            //if (checkimage)
                                            //{
                                            //     strttt += "<div style=\"float:left;width:25%;margin-bottom:5px;\" class='item-radio-display'><div style=\"float: left; width: 100%;\"><img title=\"Relaxed\" alt=\"Relaxed\" src=\"/images/Relaxed.jpg\"></div><div style=\"margin-left: 30%;\"><div class=\"iradio_flat-red\" onclick=\"imagereplace('" + StrColorPath + "');selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\" id=\"divflat-radio-" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "-" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\"></div><div style=\"margin-top: 2px;\" class='item-radio-display-text'>&nbsp;" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + StrBuy1onsale + strPrice.Replace("(", " - ").Replace(")", "") + StrbackOrderdate + "</div></div></div>";
                                            //}
                                            //else
                                            //{

                                            strttt += "<div style=\"float:left;width:25%;margin-bottom:5px;\" class='item-radio-display' onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\"><div style=\"float: left; width: 100%;\"><img title=\"Relaxed\" alt=\"Relaxed\" src=\"/images/Relaxed.jpg\"></div><div style=\"margin-left: 30%;\"><div class=\"iradio_flat-red\" onclick=\"selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\" id=\"divflat-radio-" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "-" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\"></div><div style=\"margin-top: 2px;\" class='item-radio-display-text'>&nbsp;" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + StrBuy1onsale + strPrice.Replace("(", " - ").Replace(")", "") + StrbackOrderdate + "</div></div></div>";
                                            //}
                                        }
                                        else
                                        {
                                            //if (checkimage)
                                            //{
                                            //     strttt += "<div style=\"float:left;width:100%;margin-bottom:5px;\" class='item-radio-display'><div class=\"iradio_flat-red\" onclick=\"imagereplace('" + StrColorPath + "');selecteddropdownvalue(" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "," + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + ");PriceChangeondropdown();\" id=\"divflat-radio-" + dsVariant.Tables[0].Rows[i]["VariantId"].ToString() + "-" + dsVariantvalue.Tables[0].Rows[j]["VariantValueId"].ToString() + "\"></div><div style=\"margin-top: 2px;\" class='item-radio-display-text'>&nbsp;" + dsVariantvalue.Tables[0].Rows[j]["VariantValue"].ToString() + StrBuy1onsale + strPrice.Replace("(", " - ").Replace(")", "") + StrbackOrderdate + "</div></div>";
                                            //}
                                            //else
                                            //{
                  ...
                  //  strvarinat += strmeasurement + stroptions + strmount + strtttCord + strtttCordvalue + "</div></div>";
                    if (ViewState["isRomanShade"] != null && ViewState["isRomanShade"].ToString() == "1")
                    {
                        string strrr = strvarinat;

                        // strvarinat = strmeasurement + strrr + stroptions + strmount + strtttCord + strtttCordvalue + "</div></div>";
                        if (strtttCord == "")
                        {
                            strvarinat += strmeasurement + strmount.Replace("src=\"/images/next-option.png\"", "src=\"/images/next-option.png\" style=\"display:none;\"");
                        }
                        else
                        {
                            strvarinat += strmeasurement + strmount + strtttCord + strtttCordvalue + "</div></div>";
                        }
                    ...
                    if (strsc1.Trim() != "")
                    {
                        ...
                        Page.ClientScript.RegisterStartupScript(Page.GetType(), "PriceChangeondropdown", "PriceChangeondropdown();" + strsc1 + "", true);
                    }
                    else
                    {
                        Page.ClientScript.RegisterStartupScript(Page.GetType(), "PriceChangeondropdown", "PriceChangeondropdown();", true);
                    }
                    //Page.ClientScript.RegisterStartupScript(Page.GetType(), "PriceChangeondropdown", "PriceChangeondropdown(); "+ strsccolor+"", true);
        ...
        private void GetRecentlyProduct(string ProductID, int OrgProductID)
        {
            ...
            if (dsRecently != null && dsRecently.Tables.Count > 0 && dsRecently.Tables[0].Rows.Count > 0)
            {
                ...
                for (Int32 i = 0; i < dsRecently.Tables[0].Rows.Count; i++)
                {
                    ...
                    ltrRecently.Text += " <img src='/images/view-detail.png' alt='View Detail'  style='display:none;' class='preview' width='150' height='30' title='View Detail'></a>";
                    // Bind TagName
                    ...
                    if (!string.IsNullOrEmpty(StrTagName))
                    {
                        ...
                        else
                        {
                            ...
                            if (File.Exists(Server.MapPath(AppLogic.AppConfigs("ImagePathProduct") + "icon/" + dsRecently.Tables[0].Rows[i]["imagename"].ToString())))
                            {
        ...
        private void GetYoumayalsoLikeProduct(int ProductID)
        {
            ...
            if (dsYouMay != null && dsYouMay.Tables.Count > 0 && dsYouMay.Tables[0].Rows.Count > 0)
            {
                ...
                for (Int32 i = 0; i < dsYouMay.Tables[0].Rows.Count; i++)
                {
                    ...
                    ltrYoumay.Text += " <img src='/images/view-detail.png' alt='View Detail' style='display:none;' class='preview' width='150' height='30' title='View Detail'></a>";
                    // Bind TagName
        ...
        private void GetOptinalAccesories(String SKUs)
        {
            ...
            if (dsOA != null && dsOA.Tables.Count > 0 && dsOA.Tables[0].Rows.Count > 0)
            {
                for (Int32 i = 0; i < dsOA.Tables[0].Rows.Count; i++)
                {
                    ...
                    ltrOptionalAccesories.Text += " <img src='/images/view-detail.png' alt='View Detail' style='display:none;' class='preview' width='150' height='30' title='View Detail'></a>";
                    // Bind TagName
        ...
        private void GetnextPreviousProduct()
        {
            try
            {
                ...
                {
                    dsNext = CommonComponent.GetCommonDataSet("EXEC usp_Product_GetnextpreviousProduct " + Request.QueryString["PID"].ToString() + ",1,0");
    ...
    " </object>";
        ...
        private void GetRomanMeasuringData()
        {
            DataSet ds = new DataSet();
            ds = TopicComponent.GetTopicAccordingStoreID("romanshade-measuring-guide", Convert.ToInt32(AppLogic.AppConfigs("StoreID")));
            if (ds != null && ds.Tables.Count > 0 && ds.Tables[0].Rows.Count > 0)
            {
                ltromandivmeasuring.Text = ds.Tables[0].Rows[0]["Description"].ToString().Replace("<p>", "<p style='margin-top: 0px; margin-bottom: 0px;'>");
            }
            else
            {
                ltromandivmeasuring.Text = "Comming Soon.....";
            }
        }
