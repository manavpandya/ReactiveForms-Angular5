...
namespace Solution.UI.Web
{
    public partial class subcategory : System.Web.UI.Page
    {
        ...
        private void breadcrumbs()
        {
            try
            {
                string strbreadkrum = ConfigurationComponent.GetBreadCrum(Convert.ToInt32(Request.QueryString["CatID"]), Convert.ToInt32(Request.QueryString["CatPID"]), Convert.ToInt32(1), "", 3, 0);
        ...
        private void BindProductOfSubCategory()
        {
            ...
            dsProduct = ProductComponent.GetProductDetailsByOrderPrice(Convert.ToInt32(Request.QueryString["CatID"]), Convert.ToInt32(1), ddlTopPrice.SelectedValue, ddltopcolor.SelectedValue.ToString());
            if (dsProduct != null && dsProduct.Tables.Count > 0 && dsProduct.Tables[0].Rows.Count > 0)
            {
                ...
                //      " and pc.CategoryID=" + Convert.ToInt32(Request.QueryString["CatID"]) + "" +
                //      " and isnull(Active,0) = 1 and isnull(Deleted,0) = 0").ToString();
            ...
            else
            {
                ...
                if (!IsPostBack)
                {
                    UpdatePanelUpload.Visible = false;
                }
            }
        }
        ...
        {
            DataSet dsSubcategory = CategoryComponent.GetSubCategoryByCategoryId(Convert.ToInt32(Request.QueryString["CatID"]), Convert.ToInt32(1));
        ...
        {
            DataSet dsSubcategorydetail = objCategory.getCatdetailbycatid(Convert.ToInt32(Request.QueryString["CatID"]), Convert.ToInt32(1));
            try
            {
        ...
        protected void RepProduct_ItemDataBound(object sender, RepeaterItemEventArgs e)
        {
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (hdnImgName != null && !string.IsNullOrEmpty(hdnImgName.Value.ToString()))
                {
                    ...
                    {
                        string StrQry = "Select COUNT(VariantValueID) from tb_ProductVariantValue Where isnull(VarActive,0)=1 and productid=" + ltrproductid.Text.ToString() + " and (cast(Buy1Fromdate as date) <=  cast(GETDATE() as date) and cast(Buy1Todate as date) >=cast(GETDATE() as date)) and ISNULL(Buy1Get1,0)=1 ";
                        Int32 Intcnt = Convert.ToInt32(CommonComponent.GetScalarCommonData(StrQry.ToString()));
                        if (Intcnt > 0)
                        {
                            lblTag.Text = "<img title='Buy 1 Get 1' src=\"/images/bogof.png\" alt=\"Buy 1 Get 1\" class='byu1get1list' />";
                        ...
                        else if (lblTName != null && !string.IsNullOrEmpty(lblTName.Text.ToString().Trim()) && lblTName.Text.ToString().ToLower().IndexOf("bestseller") > -1)
                        {
                ...
                HtmlContainerControl divswatchlistadd = (HtmlContainerControl)e.Item.FindControl("divswatchlistadd");
                ...
                else
                {
                    ...
                    if (dsVariant != null && dsVariant.Tables.Count > 0 && dsVariant.Tables[0].Rows.Count > 0)
                    {
                        ...
                        if (ltTitle.Text.ToString().ToLower().IndexOf("swatch") > -1)
                        {
                            aProductlink.HRef = "javascript:void(0);";
                            aProductlink.Attributes.Remove("class");
                            aProductlink.InnerHtml = "<img title=\"ADD TO CART\" alt=\"ADD TO CART\" src=\"/images/quick-view-addtocart.png\" style=\"margin-top: 5px;\">";
                            aProductlink.Attributes.Add("onclick", "InsertProductOrderSwatchAddtocartfree(" + ltrproductid.Text.ToString() + ",'" + imgAddToCart.ClientID.ToString() + "'); return false;");
                            imgAddToCart.Attributes.Add("onclick", "InsertProductOrderSwatchAddtocartfree(" + ltrproductid.Text.ToString() + ",'" + imgAddToCart.ClientID.ToString() + "'); return false;");
                            divswatchlistadd.Attributes.Remove("class");
                            divswatchlistadd.Attributes.Add("class", "img-center-swatch");
                            ltrYourPrice.Text = ltrYourPrice.Text.ToString().Replace("Starting", "Swatch");
                        }
                        else
                        {
                            aProductlink.HRef = "/" + ltrProductURL.Value.ToString();
            ...
            if (e.Item.ItemType == ListItemType.Item || e.Item.ItemType == ListItemType.AlternatingItem)
            {
                ...
                if (hdnImgName != null && !string.IsNullOrEmpty(hdnImgName.Value.ToString()))
                {
                    ...
                    if (!string.IsNullOrEmpty(ImgName) && !ImgName.ToString().ToLower().Contains("image_not_available"))
                    {
                        ...
                        else
                        {
                            ...
                            //StrQry = "Select COUNT(VariantValueID) from tb_ProductVariantValue Where productid=" + ltrproductid.Text.ToString() + " and (cast(OnSaleFromdate as date) <=  cast(GETDATE() as date) and cast(OnSaleTodate as date) >=cast(GETDATE() as date)) and ISNULL(OnSale,0)=1 ";
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        //aProductlink.Attributes.Add("onclick", "InsertProductSubcategory(" + ltrproductid.Text.ToString() + ",'" + aProductlink.ClientID.ToString() + "');");
                        innerAddtoCart.HRef = "/" + ltrProductURL.Value.ToString();
                        // imgAddToCart.Attributes.Add("onclick", "ShowModelQuick(" + ltrproductid.Text.ToString() + ",'" + aProductlink.ClientID.ToString() + "');");
                        aProductlink.HRef = "javascript:void(0);";
        ...
        private void BindColors()
        {
            ...
            try
            {
                ...
                if (Session["catId"] != null && Session["ParentCategoryId"] != null)
                {
                    strColor = Convert.ToString(CommonComponent.GetScalarCommonData("SELECT colors+',' FROM tb_product WHERE isnull(colors,'')<>'' and  isnull(Active,0)=1 and isnull(Deleted,0)=0 and ProductId in (SELECT ProductID FROM tb_productCategory WHERE (categoryId in (" + Session["catId"].ToString() + ") or categoryId in (SELECT CategoryID FROM tb_CategoryMapping WHERE ParentCategoryID=" + Session["ParentCategoryId"].ToString() + "))) FOR XML path('')"));
                }
                else if (Session["catId"] != null)
                {
                    strColor = Convert.ToString(CommonComponent.GetScalarCommonData("SELECT colors+',' FROM tb_product WHERE isnull(colors,'')<>'' and  isnull(Active,0)=1 and isnull(Deleted,0)=0 and ProductId in (SELECT ProductID FROM tb_productCategory WHERE (categoryId in (" + Session["catId"].ToString() + ") or categoryId in (SELECT CategoryID FROM tb_CategoryMapping WHERE ParentCategoryID=" + Session["catId"].ToString() + "))) FOR XML path('')"));
                }
                else if (Session["ParentCategoryId"] != null)
                {
                    strColor = Convert.ToString(CommonComponent.GetScalarCommonData("SELECT colors+',' FROM tb_product WHERE isnull(colors,'')<>'' and  isnull(Active,0)=1 and isnull(Deleted,0)=0 and ProductId in (SELECT ProductID FROM tb_productCategory WHERE (categoryId in (" + Session["ParentCategoryId"].ToString() + ") or categoryId in (SELECT CategoryID FROM tb_CategoryMapping WHERE ParentCategoryID=" + Session["ParentCategoryId"].ToString() + "))) FOR XML path('')"));
                }
            }
            catch { }
            ...
            ddltopcolor.Items.Clear();
            ddlbottomcolor.Items.Clear();
            ddltopcolor.Items.Insert(0, new ListItem("All", ""));
            ddlbottomcolor.Items.Insert(0, new ListItem("All", ""));
            if (!string.IsNullOrEmpty(strColor))
            {
                strColor = strColor.Replace(",,", ",");
                string[] str = strColor.Split(",".ToCharArray(), StringSplitOptions.RemoveEmptyEntries);
                Int32 ic = 1;
                string strallcolor = ",";
                ...
                if (str.Length > 0)
                {
                    foreach (string str1 in str)
                    {
                        ...
                        //ddltopcolor.DataSource = dsPattern;
                        //ddltopcolor.DataTextField = "SearchValue";
                        //ddltopcolor.DataValueField = "SearchValue";
                        //ddltopcolor.DataBind();
                        //ddlbottomcolor.DataSource = dsPattern;
                        //ddlbottomcolor.DataTextField = "SearchValue";
                        //ddlbottomcolor.DataValueField = "SearchValue";
                        //ddlbottomcolor.DataBind();
                        if (strallcolor.ToString().ToLower().IndexOf("," + str1.ToString().ToLower().Trim().Replace("&amp;", "&") + ",") > -1)
                        {
                        }
                        else
                        {

                            ddltopcolor.Items.Insert(ic, new ListItem(str1.ToString().Replace("&amp;", "&"), str1.ToString().Trim().Replace("&amp;", "&")));
                            ddlbottomcolor.Items.Insert(ic, new ListItem(str1.ToString().Replace("&amp;", "&"), str1.ToString().Trim().Replace("&amp;", "&")));
                            ic++;
                        }
                        strallcolor += str1.ToString().Trim().Replace("&amp;", "&") + ",";
                    }

                }
                else
                {
                    ddlbottomcolor.DataSource = null;
                    ddlbottomcolor.DataBind();
                    ddlbottomcolor.DataSource = null;
                    ddlbottomcolor.DataBind();
                }
        ...
        protected void ddltopcolor_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (RepProduct.Visible == true)
            {
                grid_view.CssClass = "grid-click";
            ...
            ddlbottomcolor.SelectedValue = ddltopcolor.SelectedValue;
            BindProductOfSubCategory();
            if (ViewState["strFeaturedimgpostback"] != null)
            {
                //ViewState["strFeaturedimg"] = "  + ViewState["strFeaturedimg"].ToString() + "});";
        ...
        protected void ddlbottomcolor_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (RepProduct.Visible == true)
            {
                grid_view.CssClass = "grid-click";
            ...
            ddltopcolor.SelectedValue = ddlbottomcolor.SelectedValue;
            BindProductOfSubCategory();
            if (ViewState["strFeaturedimgpostback"] != null)
            {
                //ViewState["strFeaturedimg"] = "  + ViewState["strFeaturedimg"].ToString() + "});";
