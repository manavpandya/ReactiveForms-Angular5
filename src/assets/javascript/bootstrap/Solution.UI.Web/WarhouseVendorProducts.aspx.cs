...
{
    public partial class WarhouseVendorProducts : System.Web.UI.Page
    {
        /// <summary>
        /// Page Load Event
        /// </summary>
        ...
        public DataSet GetVendorProductsforWarehouse(int Pono)
        {
            return CommonComponent.GetCommonDataSet("SELECT  dbo.tb_Product.Name, dbo.tb_PurchaseOrderItems.Quantity, dbo.tb_PurchaseOrderItems.PONumber,dbo.tb_PurchaseOrder.OrderNumber," +
                   ...
                   " dbo.tb_PurchaseOrder.OrderNumber,dbo.tb_PurchaseOrderItems.TrackingNumber, dbo.tb_PurchaseOrder.PODate,dbo.tb_Vendor.Phone,isnull(tb_PurchaseOrderItems.ShippedVia,'') as ShippedVia,isnull(tb_PurchaseOrderItems.ShippedOn,'') as ShippedOn,ISNULL(tb_PurchaseOrderItems.ProductOption,'') as ProductOption" +
                   ...
                   " rep.RefProductID = dbo.tb_PurchaseOrderItems.ProductID AND rep.VendorQuoteRequestID = dbo.tb_PurchaseOrderItems.VendorQuoteRequestID " +
        ...
        protected void btnsubmit_Click(object sender, ImageClickEventArgs e)
        {
            ...
            if (falg == true)
            {
                try
                {
                    CommonComponent.ExecuteCommonData("INSERT INTO tb_Timestamplog(Createdby,type,refnumber,orderNumber,VendorName,Description) VALUES (-1,27," + Convert.ToInt32(Request.QueryString["pono"].ToString()) + "," + Convert.ToInt32(0) + ",'" + ltvname.Text.ToString().Replace("'", "''") + "','" + txtNotes.Text.Trim().Replace("'", "''") + "')");
        ...
        public void SendMail()
        {
            ...
            try
            {
                ...
                if (dsMailTemplate != null && dsMailTemplate.Tables.Count > 0 && dsMailTemplate.Tables[0].Rows.Count > 0)
                {
                    ...
                    ORDERCART += ("<th valign='middle' align='Center' style='width:50%;background-color: #F6F5F5; border: 1px solid #DFDFDF;color: #212121; font-size: 12px; font-weight: bold;text-align:left;' align='left'>Product Name</th>");
                    ...
                    ORDERCART += ("<th valign='middle' style='width:10%;background-color: #F6F5F5; border: 1px solid #DFDFDF;color: #212121; font-size: 12px; font-weight: bold; padding-left: 5px;' align='center'>Quantity</th>");
                    ORDERCART += ("<th valign='middle' style='width:10%;background-color: #F6F5F5; border: 1px solid #DFDFDF;color: #212121; font-size: 12px; font-weight: bold; padding-left: 5px;text-align:right;' align='right'>Price</th>");
                    ...
                    foreach (GridViewRow row in gvVendor.Rows)
                    {
                        try
                        {
                            ...
                            if (ddlstt.Visible == true)
                            {
                                ...
                                {
                                    ORDERCART += ("<TR>");
                                    if (!string.IsNullOrEmpty(lblProductOption.Text.ToString()))
                                        ORDERCART += ("<td style='background: #FFFFFF;border:1px solid #DADADA; color: #212121; font-size: 12px; line-height: 18px;max-height: 30px; padding: 5px 8px;text-align:left;' align='left' valign='middle' style='width:60%' >" + Convert.ToString(lblName.Text.Trim() + "<br/> " + lblProductOption.Text.ToString().Replace("\r\n", "<br/>")) + "</td>");
                                    else
                                        ORDERCART += ("<td style='background: #FFFFFF;border:1px solid #DADADA; color: #212121; font-size: 12px; line-height: 18px;max-height: 30px; padding: 5px 8px;text-align:left;' align='left' valign='middle' style='width:60%' >" + lblName.Text.Trim() + "</td>");

                                    ORDERCART += ("<td style='background: #FFFFFF;border:1px solid #DADADA; color: #212121; font-size: 12px; line-height: 18px;max-height: 30px; padding: 5px 8px;text-align:left;' align='left' valign='middle' style='width:20%' >" + lblSKU.Text.Trim() + "</td>");
                                    ORDERCART += ("<td style='background: #FFFFFF;border:1px solid #DADADA; color: #212121; font-size: 12px; line-height: 18px;max-height: 30px; padding: 5px 8px;text-align:center;' valign='middle' style='width: 20%;text-align:center;'>" + txtQuantity.Text.Trim() + "</td>");
                                    ORDERCART += ("<td style='background: #FFFFFF;border:1px solid #DADADA; color: #212121; font-size: 12px; line-height: 18px;max-height: 30px; padding: 5px 8px;text-align:right;' valign='middle' style='width: 20%;text-align:right;'>$" + String.Format("{0:0.00}", Convert.ToDecimal(txtPrice.Text.Trim())) + "</td>");
                                    ORDERCART += ("<td style='background: #FFFFFF;border:1px solid #DADADA; color: #212121; font-size: 12px; line-height: 18px;max-height: 30px; padding: 5px 8px;text-align:center;' valign='middle' style='width: 20%;text-align:center;'>" + ddlstt.SelectedItem.Text.ToString() + "</td>");
                                    ORDERCART += ("</tr>");
                                }
                            }
                        }
        ...
        private void SendShippedMail(int OrderNumber, String Cart, string ToID)
        {
            try
            {
                ...
                sw.Append("<img src=\"" + GetAppConfigByName("LIVE_SERVER", 1) + "/images/Store_" + Convert.ToString(AppConfig.StoreID.ToString()) + ".png\" />");
                sw.Append("</td>");
                sw.Append("</tr>");
                sw.Append("<tr>");
                sw.Append("<td>&nbsp;</td></tr>");
