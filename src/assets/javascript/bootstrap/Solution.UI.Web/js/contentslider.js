...
var csloadedobjects = "" //Variable to store file names of .js/.css files already loaded (if Ajax is used)
    ...
    function ContentSlider(sliderid, autorun, customPaginateText, customNextText) {
        //percy--set toggle button start
        hide_play()
        //percy--set toggle button end
        ...
        if (typeof autorun == "number" && autorun > 0) //if autorun parameter (int_miliseconds) is defined, fire auto run sequence
            window[sliderid + "timer"] = setTimeout(function () {
                ContentSlider.autoturnpage(sliderid, autorun)
            }, autorun)
    }
...
ContentSlider.buildpagination = function (sliderid) {
    ...
    var paginatediv_percy = document.getElementById("paginate-new")
    paginatediv_percy.onclick = function () { //cancel auto run sequence (if defined) when user clicks on pagination DIV
...
ContentSlider.autoturnpage = function (sliderid, autorunperiod) {
    ...
    window[sliderid + "timer"] = setTimeout(function () {
        ContentSlider.autoturnpage(sliderid, autorunperiod)
    }, autorunperiod)
}
...
ContentSlider.fadeup = function (sliderid) {
    ...
    if (targetobject && targetobject.currentopacity < 1) {
        ...
        window[sliderid + "fadetimer"] = setTimeout(function () {
            ContentSlider.fadeup(sliderid)
        }, 100)
    }
}
