...
pgwjs = {
    checkNavbar: function () {
        var navbar_width = $('header nav').width();

        function calcContentWidth() {
            var navbar_content_width = 0;
            $('#mnct').show();
            $('.min-mnli').not('.hide').each(function () {
                navbar_content_width += $(this).width();
            });
            return (navbar_content_width + 50);
        }

        function switchMenu(type) {
            if (type == 1) {
                $('#mnli').removeClass('mob hide');
                $('#mbmn').addClass('hide');
            } else {
                $('#mnli').addClass('mob hide');
                $('#mbmn').removeClass('hide');
            }
            $('#mbmn a').removeClass('act');
        }
        if (calcContentWidth() > navbar_width) {
            switchMenu(0);
        } else {
            switchMenu(1);
            if (calcContentWidth() > navbar_width) {
    ...
    displayMobileMenu: function () {
        if ($('#mbmn a').hasClass('act')) {
            pgwjs.disableMobileMenu();
            return false;
        }
        $('#mbmn a').addClass('act');
        $('#mnli').removeClass('hide');
    },
